...
import type { CompiledProgress, CompiledTopPhase, SprintStats } from '../types.js';
...
function assertIncludes(str: string, substring: string, message?: string): void {
  ...
  if (!str.includes(substring)) throw new Error(msg);
}
...
function createTestChartData(overrides: Partial<ChartData> = {}): ChartData {
  return {
    completed: 5,
    pending: 3,
    failed: 2,
...
function createTestChartOptions(overrides: Partial<ChartOptions> = {}): ChartOptions {
  return {
    width: 200,
    height: 200,
    showLegend: true,
...
function createTimelineTestData(): TimelineEntry[] {
  return [
    {
      id: 'phase-1',
      label: 'Setup',
...
test('progress chart module exports createProgressChart function', () => {
  assert(typeof createProgressChart === 'function', 'createProgressChart should be a function');
});

test('progress chart module exports createPieChart function', () => {
  assert(typeof createPieChart === 'function', 'createPieChart should be a function');
});

test('progress chart module exports createProgressBar function', () => {
  assert(typeof createProgressBar === 'function', 'createProgressBar should be a function');
});

test('progress chart module exports createTimelineChart function', () => {
  assert(typeof createTimelineChart === 'function', 'createTimelineChart should be a function');
});

test('progress chart module exports formatChartLabel function', () => {
  assert(typeof formatChartLabel === 'function', 'formatChartLabel should be a function');
});
...
test('generates valid SVG - createProgressChart returns SVG string', () => {
  ...
  const svg = createProgressChart(data, options);
...
test('generates valid SVG - SVG contains chart elements', () => {
  ...
  const hasChartElements =
    svg.includes('<rect') || svg.includes('<circle') || svg.includes('<path');
...
test('timeline with timestamps - createTimelineChart generates timeline', () => {
  ...
  const svg = createTimelineChart(entries, options);
...
test('PDF includes chart - PDF contains chart when includeCharts is true', async () => {
  ...
  const pdfBuffer = await createPdfDocument(progress, options);
...
test('PDF includes chart - no chart when includeCharts is false', async () => {
  const progress = createTestProgress();
  const withChart: PdfOptions = { includeCharts: true };
  const withoutChart: PdfOptions = { includeCharts: false };

  const pdfWithChart = await createPdfDocument(progress, withChart);
  const pdfWithoutChart = await createPdfDocument(progress, withoutChart);

  // PDF without chart should be smaller
  assert(
    pdfWithoutChart.length < pdfWithChart.length,
    'PDF without chart should be smaller than PDF with chart'
  );
});
...
test('progress bar renders - createProgressBar returns SVG', () => {
  ...
  const svg = createProgressBar(data, options);
...
test('createPieChart generates donut chart', () => {
  ...
  const svg = createPieChart(segments, options);
...
test('createPieChart handles single segment (full circle)', () => {
  const segments: ChartSegment[] = [{ value: 100, color: '#2E7D32', label: 'Completed' }];
  const options = createTestChartOptions({ type: 'pie' });
...
test('createProgressChart defaults to pie chart type', () => {
  const data = createTestChartData();
  const options: ChartOptions = { width: 200, height: 200 }; // No type specified
