...
test('BUG-005: network error containing "error" should match network, not logic', () => {
  ...
  const result = classifyError(1, 'network error occurred');

  assertEqual(result.category, 'network' as ErrorCategory,
    'network error should be classified as network category');
});
...
test('BUG-005: validation failed should match validation, not logic', () => {
  ...
  const result = classifyError(1, 'validation failed for input');

  assertEqual(result.category, 'validation' as ErrorCategory,
    'validation failed should be classified as validation category');
});
...
test('BUG-005: connection failed should match network, not logic', () => {
  ...
  const result = classifyError(1, 'connection failed');

  assertEqual(result.category, 'network' as ErrorCategory,
    'connection failed should be classified as network category');
});
...
test('BUG-005: rate limit error should match rate-limit, not logic', () => {
  ...
  const result = classifyError(1, 'rate limit error: 429');

  assertEqual(result.category, 'rate-limit' as ErrorCategory,
    'rate limit error should be classified as rate-limit category');
});
...
test('BUG-005: timeout exception should match timeout, not logic', () => {
  ...
  const result = classifyError(1, 'timeout exception occurred');

  assertEqual(result.category, 'timeout' as ErrorCategory,
    'timeout exception should be classified as timeout category');
});
...
test('BUG-005: DNS failed error should match network, not logic', () => {
  ...
  const result = classifyError(1, 'DNS failed error');

  assertEqual(result.category, 'network' as ErrorCategory,
    'DNS failed error should be classified as network category');
});
...
test('BUG-005: schema validation error should match validation, not logic', () => {
  ...
  const result = classifyError(1, 'schema validation error');

  assertEqual(result.category, 'validation' as ErrorCategory,
    'schema validation error should be classified as validation category');
});
...
test('BUG-005: generic error should match logic category', () => {
  ...
  const result = classifyError(1, 'unknown error in processing');

  assertEqual(result.category, 'logic' as ErrorCategory,
    'generic error should be classified as logic category');
});

test('BUG-005: generic failure should match logic category', () => {
  // A generic failure message with no specific category patterns
  const result = classifyError(1, 'operation failed unexpectedly');

  assertEqual(result.category, 'logic' as ErrorCategory,
    'generic failure should be classified as logic category');
});
...
test('BUG-005: specific category should have high confidence', () => {
  const result = classifyError(1, 'ECONNREFUSED');

  assertEqual(result.category, 'network' as ErrorCategory, 'should be network');
...
test('BUG-005: logic category should have lower confidence', () => {
  const result = classifyError(1, 'some random error');

  assertEqual(result.category, 'logic' as ErrorCategory, 'should be logic');
...
test('BUG-005: loop order ensures specific patterns are checked before logic fallback', () => {
  ...
  const networkResult = classifyError(1, 'network error');
  assertEqual(networkResult.category, 'network' as ErrorCategory,
    'should classify as network due to priority loop order');
});
...
test('BUG-005: REGRESSION - messages with multiple category keywords should match most specific', () => {
  ...
  const result1 = classifyError(1, 'connection timeout error');
  ...
  assertEqual(result1.category, 'timeout' as ErrorCategory,
    'connection timeout error should match timeout');
  ...
  const result2 = classifyError(1, 'rate limit validation error');
  assertEqual(result2.category, 'rate-limit' as ErrorCategory,
    'rate limit validation error should match rate-limit');
});
...
test('BUG-005: CODE CLARITY - implementation is clean and straightforward', () => {
  ...
  const result = classifyError(1, 'validation error occurred');
  assertEqual(result.category, 'validation' as ErrorCategory,
    'validation-specific pattern should match before logic fallback');
});
