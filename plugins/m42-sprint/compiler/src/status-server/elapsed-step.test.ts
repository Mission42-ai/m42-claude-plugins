...
test('Scenario 5: countTotalSteps returns correct total', () => {
  ...
  const transforms = require('./transforms.js') as { countTotalSteps?: (p: CompiledProgress) => number };

  if (!transforms.countTotalSteps) {
    throw new Error(
      'RED: countTotalSteps function does not exist in transforms.ts. ' +
...
test('Scenario 6: SprintHeader includes totalSteps', () => {
  ...
  const header = update.header as { totalSteps?: number };
...
test('Scenario 7: SprintHeader includes currentStep', () => {
  ...
  const header = update.header as { currentStep?: number };
...
test('Scenario 8: page contains sprint-timer element', () => {
  ...
  assert(
    html.includes('sprint-timer') || html.includes('id="sprint-timer"'),
    'Page HTML should contain sprint-timer element. ' +
    'Add a div with id="sprint-timer" in the header section.'
  );

  // Check for timer icon (clock/stopwatch emoji or icon)
  assert(
    html.includes('timer') || html.includes('clock') || html.includes('stopwatch'),
    'Page should include timer-related styling or icon class'
  );

  // Check for JavaScript that updates timer every second
  assert(
    html.includes('setInterval') && (html.includes('1000') || html.includes('timer')),
    'Page should include JavaScript setInterval for timer updates'
  );

  // Check for HH:MM:SS formatting function or pattern
  assert(
    html.includes('formatTime') ||
    html.includes('HH:MM:SS') ||
    html.includes('toHHMMSS') ||
    (html.includes('hours') && html.includes('minutes') && html.includes('seconds')),
    'Page should include HH:MM:SS time formatting logic'
  );
});
...
test('Scenario 8b: page contains step progress counter element', () => {
  ...
  assert(
    ...
    (html.includes('Step') && html.includes('of')),
    'Page HTML should contain step progress counter element (e.g., "Step X of Y")'
  );
});
...
test('Edge case: calculateElapsed handles various time ranges', () => {
  ...
  const sec45Elapsed = calculateElapsed(sec45Ago);
  assert(sec45Elapsed.includes('45s') || sec45Elapsed.includes('44s') || sec45Elapsed.includes('46s'),
    `45 seconds should format as ~45s, got: ${sec45Elapsed}`);

  // 90 minutes ago
  const min90Ago = new Date(now.getTime() - 90 * 60 * 1000).toISOString();
  const min90Elapsed = calculateElapsed(min90Ago);
  assert(min90Elapsed.includes('1h 30m') || min90Elapsed.includes('1h 29m') || min90Elapsed.includes('1h 31m'),
    `90 minutes should format as ~1h 30m, got: ${min90Elapsed}`);
});
