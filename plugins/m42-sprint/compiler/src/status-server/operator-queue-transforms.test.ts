...
// Mock CompiledProgress with operator-queue
...
function createMockProgressWithQueue(): MockProgress {
  return {
...
function createMockBacklogFile(): { items: BacklogItem[] } {
  return {
    items: [
      {
        id: 'back_001',
...
test('toOperatorQueueData separates pending from history', async () => {
  ...
  const result = toOperatorQueueData(progress as any, backlog);
...
test('toOperatorQueueData sorts pending by priority', async () => {
  ...
  const result = toOperatorQueueData(progress as any, { items: [] });
...
test('calculateQueueStats counts all statuses correctly', async () => {
  ...
  const queue: QueuedRequest[] = [
    { id: '1', status: 'pending' } as QueuedRequest,
    { id: '2', status: 'pending' } as QueuedRequest,
    { id: '3', status: 'approved' } as QueuedRequest,
    { id: '4', status: 'rejected' } as QueuedRequest,
    { id: '5', status: 'deferred' } as QueuedRequest,
    { id: '6', status: 'deferred' } as QueuedRequest,
    { id: '7', status: 'backlog' } as QueuedRequest,
  ];

  const backlog = [{ id: 'b1' } as BacklogItem, { id: 'b2' } as BacklogItem];
...
test('formatRelativeTime returns just now for recent times', async () => {
  ...
  assert(
    result.includes('just now') || result.includes('0') || result.includes('now'),
    'should show "just now" or similar for current time'
  );
});
...
test('applyManualDecision marks source as manual', async () => {
  ...
  assert(
    ...
    result.decision?.reasoning.includes('Manual') ||
    (result as any)['decision-source'] === 'manual',
    'should mark decision as manual'
  );
});
...
test('sortByPriority orders correctly', async () => {
  ...
  const requests: QueuedRequest[] = [
    { id: '1', priority: 'low' } as QueuedRequest,
    { id: '2', priority: 'critical' } as QueuedRequest,
    { id: '3', priority: 'medium' } as QueuedRequest,
    { id: '4', priority: 'high' } as QueuedRequest,
  ];
