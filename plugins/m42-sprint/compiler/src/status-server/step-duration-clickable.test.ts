...
import { buildPhaseTree, toStatusUpdate, calculateElapsed } from './transforms.js';
import type { CompiledProgress, PhaseTreeNode } from './status-types.js';
 ...
 */
function createInProgressStepProgress(): CompiledProgress {
  const now = new Date();
...
test('BUG-007 DETECTION: in-progress steps should have duration available', () => {
  ...
  assert(
    inProgressStep.elapsed !== undefined,
    `BUG-007 DETECTED: In-progress step has no duration. ` +
...
test('BUG-007 DETECTION: in-progress sub-phases should have duration available', () => {
  ...
  assert(
    inProgressSubPhase.elapsed !== undefined,
    `BUG-007 DETECTED: In-progress sub-phase has no duration. ` +
...
test('BUG-007 DETECTION: toStatusUpdate provides duration for in-progress nodes', () => {
  ...
  assert(
    step.elapsed !== undefined && step.elapsed.length > 0,
    `BUG-007: StatusUpdate missing duration for in-progress step. ` +
...
test('calculateElapsed works for in-progress (no end time)', () => {
  ...
  const elapsed = calculateElapsed(fiveMinutesAgo.toISOString());

  // Should be approximately "5m 0s" (allow some tolerance)
  assert(
    elapsed.includes('5m') || elapsed.includes('4m'),
    `calculateElapsed should compute ~5m for a start time 5 minutes ago, got: ${elapsed}`
  );
});
...
test('BUG-007 DETECTION: step rows should be clickable', () => {
  ...
  assert(
    hasClickableCursor,
    `BUG-007 DETECTED: Step rows (.tree-node-content) should have cursor: pointer style ` +
...
test('BUG-007 DETECTION: step rows should have click handlers for log viewing', () => {
  ...
  const hasRowClickHandler =
    html.includes("querySelectorAll('.tree-node-content')") &&
...
test('BUG-007 DETECTION: clickable steps should have hover indication', () => {
  ...
  const hasHoverStyles =
    html.includes('.tree-node-content:hover') &&
    (html.includes('background') || html.includes('text-decoration'));

  assert(
    hasHoverStyles,
    `BUG-007 DETECTED: Step rows should have :hover styles to indicate clickability. ` +
...
test('View Log button is still rendered for leaf nodes', () => {
  ...
  assert(
    html.includes('log-viewer-toggle') && html.includes('View Log'),
    'View Log button should still be present in the HTML template'
  );
});
