...
test('generateNavigationBar should include sprint dropdown with onchange handler', async () => {
  ...
  assert(
    html.includes('sprint-select'),
    'HTML should contain sprint-select element'
  );
  assert(
    html.includes('onchange'),
    'Dropdown should have onchange handler'
  );
});
...
test('dropdown onchange should navigate to /sprint/{id}', async () => {
  ...
  assert(
    html.includes("window.location.href") || html.includes("location.href"),
    'Dropdown onchange should set window.location.href'
  );
  assert(
    html.includes("/sprint/"),
    'Navigation should go to /sprint/ path'
  );
});
...
test('dropdown change should close existing SSE connection', async () => {
  ...
  const hasSSECloseLogic =
    html.includes('closeEventSource') ||
  ...
  assert(
    hasSSECloseLogic,
    'JavaScript should close SSE connection before navigation (RED: expected to fail)'
  );
});
...
test('dropdown change should show loading indicator', async () => {
  ...
  assert(
    hasLoadingElement,
    'Page should have a loading indicator element (RED: expected to fail)'
  );
});
...
test('loading indicator should be shown when dropdown changes', async () => {
  ...
  const showsLoadingOnChange =
    html.includes('showLoading') ||
  ...
  assert(
    showsLoadingOnChange,
    'Dropdown change should show loading indicator (RED: expected to fail)'
  );
});
