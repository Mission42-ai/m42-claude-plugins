...
import { isActivityEvent, type ActivityEvent, isVerbosityLevel } from './activity-types.js';
...
test('Scenario 1: isActivityEvent accepts assistant event type', () => {
  ...
  assert(
    isActivityEvent(assistantEvent),
    `isActivityEvent should accept type='assistant'. ` +
...
test('Scenario 1: ActivityEvent type union includes assistant', () => {
  ...
  assert(
    ...
    `ActivityEvent type union should include 'assistant'. ` +
...
test('Scenario 2: isActivityEvent validates text field for assistant type', () => {
  ...
  assert(
    isActivityEvent(assistantEventWithText),
    `isActivityEvent should accept assistant event with text field. ` +
    `This tests that text is a valid optional field for assistant events.`
  );

  // Verify text field is accessible
  if (isActivityEvent(assistantEventWithText)) {
    // This assertion will fail at compile time until text is added to ActivityEvent
    assert(
      (assistantEventWithText as { text?: string }).text === 'Hello user, I am working on your request.',
      'Text field should be accessible on validated event'
    );
  }
});
...
test('Scenario 2: isActivityEvent validates isThinking field for assistant type', () => {
  ...
  assert(
    isActivityEvent(thinkingEvent),
    `isActivityEvent should accept assistant event with isThinking field. ` +
    `isThinking indicates the assistant is currently processing (typing indicator).`
  );

  // Verify isThinking field is accessible
  if (isActivityEvent(thinkingEvent)) {
    assert(
      (thinkingEvent as { isThinking?: boolean }).isThinking === true,
      'isThinking field should be accessible on validated event'
    );
  }
});
...
test('Scenario 2: isActivityEvent accepts assistant event with both text and isThinking', () => {
  ...
  assert(
    isActivityEvent(fullAssistantEvent),
    `isActivityEvent should accept assistant event with both text and isThinking fields`
  );
});
...
test('Scenario 2: isActivityEvent rejects assistant event with invalid text type', () => {
  ...
  const isValid = isActivityEvent(invalidTextEvent);
...
test('Scenario 2: isActivityEvent rejects assistant event with invalid isThinking type', () => {
  ...
  const isValid = isActivityEvent(invalidThinkingEvent);
...
test('Backward compatibility: tool events still validate correctly', () => {
  ...
  assert(
    isActivityEvent(toolEvent),
    `Tool events should still be valid after adding assistant support`
  );
});
...
test('Backward compatibility: tool events with all fields validate', () => {
  ...
  assert(
    isActivityEvent(fullToolEvent),
    `Full tool events should validate correctly`
  );
});
...
test('Scenario 1: Assistant events can have any verbosity level', () => {
  const levels: Array<'minimal' | 'basic' | 'detailed' | 'verbose'> = ['minimal', 'basic', 'detailed', 'verbose'];
  ...
  for (const level of levels) {
    ...
    assert(
      isActivityEvent(event),
      `Assistant event with level='${level}' should be valid`
    );
  }
});
