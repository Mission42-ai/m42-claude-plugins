...
// ============================================================================
// Types (matching page.ts implementation)
// ============================================================================

type SprintStatus = 'not-started' | 'in-progress' | 'completed' | 'failed' | 'blocked' | 'needs-human';
...
function test(name: string, fn: () => void): void {
  try {
    ...
    testResults.push({ name, passed: false, error: error as Error });
  }
}
...
class NotificationController {
  private previousSprintStatus: SprintStatus | null = null;
  private notificationPreferences: NotificationPreferences;
  public soundPlayedCount = 0;
  public notificationsSentCount = 0;
   ...
   */
  handleStatusUpdate(update: StatusUpdate): { notified: boolean; reason: string } {
...
test('BUG-005: Sound should NOT play when opening already-completed sprint (FAILING TEST)', () => {
  ...
  const completedSprintUpdate: StatusUpdate = {
    header: {
      status: 'completed',
  ...
  assertEqual(
    controller.soundPlayedCount,
    0,
    `BUG-005: Sound should NOT play on initial load of completed sprint.\n` +
...
test('Sound SHOULD play when sprint transitions from in-progress to completed', () => {
  ...
  assertEqual(
    result.notified,
    true,
    'Notification SHOULD trigger when sprint transitions to completed'
  );
...
test('BUG-005: Sound should NOT play when opening already-failed sprint (FAILING TEST)', () => {
  ...
  const failedSprintUpdate: StatusUpdate = {
...
test('In-progress sprint initial load should NOT trigger notification', () => {
  ...
  const inProgressUpdate: StatusUpdate = {
