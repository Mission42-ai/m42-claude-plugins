...
function getWorktreeName(sprint: SprintSummary): string {
...
function getWorktreeNameFromApi(isMain: boolean, directoryName: string): string {
...
test('extractWorktreeName returns directory basename when worktree info is missing', () => {
  ...
  assertEqual(
    worktreeName,
    'm42-claude-plugins',
    'Without worktree info, should return directory basename'
  );
});
...
test('filter mismatch occurs when worktree info is missing (documenting old bug behavior)', () => {
  ...
  assertEqual(
    filterDropdownValue,
    'main',
    'API returns "main" for main worktree'
  );
  assertEqual(
    rowWorktree,
    'm42-claude-plugins',
    'Path extraction returns directory basename'
  );
  // They don't match - this is why the fix uses includeWorktreeInfo: true
  assert(
    filterDropdownValue !== rowWorktree,
    'Without worktree info, filter values mismatch (this is why the fix is needed)'
  );
});
...
test('filter works correctly when worktree info is included', () => {
  ...
  assertEqual(
    ...
    sprints.length,
    `All sprints should be visible when worktree info is present`
  );
});

test('linked worktree filter works with worktree info', () => {
  const filterValue: string = 'feature-branch';
  ...
  assert(
    shouldShow,
    `Linked worktree filter should work: filter="${filterValue}", rowWorktree="${rowWorktree}"`
  );
});
...
test('"All Worktrees" filter always shows all sprints', () => {
  ...
  assertEqual(
    visibleSprints.length,
    sprints.length,
    'All sprints should be visible when "all" is selected'
  );
});
...
test('mixed worktree filtering works correctly', () => {
  ...
  const featureFilter: string = 'feature-a';
