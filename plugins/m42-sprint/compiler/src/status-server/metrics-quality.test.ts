...
import { MetricsAggregator, AggregateMetrics, TrendDataPoint } from './metrics-aggregator.js';
...
function createSprintSummary(overrides: Partial<SprintSummary> = {}): SprintSummary {
  const now = new Date();
  return {
...
function createSprintSet(configs: Array<{ status: string; durationMs: number; date: string }>): SprintSummary[] {
  return configs.map((cfg, i) => {
    ...
    return createSprintSummary({
      ...
      status: cfg.status as SprintSummary['status'],
...
test('AggregateMetrics should include sprint health status (healthy/warning/critical)', () => {
  ...
  expect((metrics as any).healthStatus).toBeOneOf(['healthy', 'warning', 'critical']);
});
...
test('AggregateMetrics should include success rate trend (improving/declining/stable)', () => {
  ...
  expect((metrics as any).successRateTrend).toBeOneOf(['improving', 'declining', 'stable']);
});
...
test('AggregateMetrics should flag duration anomalies', () => {
  ...
  expect((metrics as any).durationAnomalies).toBeGreaterThan(0);
});
...
test('AggregateMetrics should provide categorized metrics (velocity, quality, efficiency)', () => {
  ...
  const categories = (metrics as any).categories;
...
test('AggregateMetrics should include period-over-period comparison', () => {
  ...
  const comparison = (metrics as any).comparison;
...
test('AggregateMetrics should flag metrics needing attention with severity levels', () => {
  ...
  const alerts = (metrics as any).alerts;
  expect(Array.isArray(alerts)).toBe(true);

  // Should have at least one alert for the poor success rate
  const hasSuccessAlert = alerts.some((a: any) =>
    a.metric === 'successRate' && a.severity === 'critical'
  );
...
test('AggregateMetrics workflowStats should include per-workflow success rates', () => {
  ...
  expect((buildFeatureStats as any).successRate).toBe(67); // 2/3 success
  ...
  expect((fixBugStats as any).successRate).toBe(33); // 1/3 success
});
...
test('TrendDataPoint should include successRate percentage', () => {
  ...
  expect((jan18 as any).successRate).toBe(67); // 2/3 success on Jan 18
});
...
test('AggregateMetrics should include efficiency metrics (steps per hour)', () => {
  ...
  expect((metrics as any).averageStepsPerHour).toBeGreaterThan(0);
});
...
test('AggregateMetrics should include actionable summary insights', () => {
  ...
  const insights = (metrics as any).insights;
