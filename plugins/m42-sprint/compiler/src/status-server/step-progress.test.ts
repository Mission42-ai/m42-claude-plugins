...
import { buildPhaseTree, toStatusUpdate } from './transforms.js';
 ...
 */
function createMixedStatusProgress(): CompiledProgress {
  return {
  ...
  } as unknown as CompiledProgress;
}
 ...
 */
function createAllPendingProgress(): CompiledProgress {
  return {
...
test('buildPhaseTree preserves mixed step statuses', () => {
  ...
  assert(
    uniqueStatuses.size === 3,
    `Expected 3 unique step statuses, got ${uniqueStatuses.size}: ${Array.from(uniqueStatuses).join(', ')}`
  );
});
...
test('BUG-001 DETECTION: steps should NOT all show pending when sprint is in-progress', () => {
  ...
  assert(
    pendingCount < steps.length,
    `BUG-001 DETECTED: All ${steps.length} steps show 'pending' status even though ` +
...
test('toStatusUpdate delivers correct step statuses for rendering', () => {
  ...
  assertEqual(
    phase.children![0].status,
    'completed',
    'First step status for rendering (should produce tree-icon completed)'
  );
  assertEqual(
    phase.children![1].status,
    'in-progress',
    'Second step status for rendering (should produce tree-icon in-progress)'
  );
  assertEqual(
    phase.children![2].status,
    'pending',
    'Third step status for rendering (should produce tree-icon pending)'
  );
});
...
test('sub-phase statuses are preserved for rendering', () => {
  ...
  assertEqual(
    step0.children![0].status,
    'completed',
    'Step 0 sub-phase should be completed'
  );
  assertEqual(
    step1.children![0].status,
    'in-progress',
    'Step 1 sub-phase should be in-progress'
  );
  assertEqual(
    step2.children![0].status,
    'pending',
    'Step 2 sub-phase should be pending'
  );
...
test('step statuses map to valid CSS icon classes', () => {
  ...
  for (const step of steps) {
    assert(
      validStatusClasses.has(step.status),
      `Step status "${step.status}" is not a valid CSS class. ` +
...
test('data format supports correct tree-icon class generation', () => {
  ...
  assert(
    simulatedHtml.includes('tree-icon completed'),
    'Rendered HTML should include "tree-icon completed"'
  );
  assert(
    simulatedHtml.includes('tree-icon in-progress'),
    'Rendered HTML should include "tree-icon in-progress"'
  );
  assert(
    simulatedHtml.includes('tree-icon pending'),
    'Rendered HTML should include "tree-icon pending"'
  );
  ...
  assert(
    !allSameStatus,
    'BUG-001: All steps have the same status - visual indicators would all look identical'
  );
});
