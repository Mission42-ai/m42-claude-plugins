...
// ============================================================================
// Simulate the client-side renderTreeNode function
// This mirrors the logic in page.ts to generate tree node HTML
// ============================================================================

interface PhaseTreeNode {
  id: string;
  label: string;
  status: 'pending' | 'in-progress' | 'completed' | 'failed' | 'blocked' | 'skipped';
 ...
 */
function renderTreeNode(node: PhaseTreeNode, parentPath = ''): string {
...
test('BUG-001: Completed step should render with "completed" CSS class', async () => {
  ...
  assert(
    html.includes('class="tree-icon completed"'),
    `Expected HTML to contain 'class="tree-icon completed"', but got: ${html}`
  );

  // Should NOT have pending class
  assert(
    !html.includes('class="tree-icon pending"'),
    'Completed step should not have pending class'
  );
});
...
test('BUG-001: In-progress step should render with "in-progress" CSS class', async () => {
  ...
  assert(
    html.includes('class="tree-icon in-progress"'),
    `Expected HTML to contain 'class="tree-icon in-progress"', but got: ${html}`
  );
});
...
test('BUG-001: Pending step should render with "pending" CSS class', async () => {
  ...
  assert(
    html.includes('class="tree-icon pending"'),
    `Expected HTML to contain 'class="tree-icon pending"', but got: ${html}`
  );
});
...
test('BUG-001: Failed step should render with "failed" CSS class', async () => {
  ...
  assert(
    html.includes('class="tree-icon failed"'),
    `Expected HTML to contain 'class="tree-icon failed"', but got: ${html}`
  );
});
...
test('BUG-001: Mixed status tree should show different indicators for each step', async () => {
  ...
  assert(
    ...
    `Expected 1 completed indicator, got ${completedMatches.length}`
  );

  assert(
    inProgressMatches.length === 2,
    `Expected 2 in-progress indicators (phase + step), got ${inProgressMatches.length}`
  );

  assert(
    pendingMatches.length === 1,
    `Expected 1 pending indicator, got ${pendingMatches.length}`
  );
});
...
test('BUG-001: All status values should produce valid CSS class names', async () => {
  const validStatuses = ['pending', 'in-progress', 'completed', 'failed', 'blocked', 'skipped'] as const;
  ...
  for (const status of validStatuses) {
    ...
    assert(
      html.includes(`class="tree-icon ${status}"`),
      `Status "${status}" should produce 'class="tree-icon ${status}"', but got: ${html}`
    );

    // Ensure no "undefined" in the class
    assert(
      !html.includes('undefined'),
      `Status "${status}" should not produce "undefined" in HTML`
    );
  }
});
...
test('BUG-001: E2E - buildPhaseTree output renders with correct status classes', async () => {
  ...
  const tree = buildPhaseTree(progress as any);

  // Render the tree to HTML
  let html = '';
  for (const node of tree) {
    html += renderTreeNode(node as PhaseTreeNode);
  }
  ...
  assert(
    html.includes('class="tree-icon completed"'),
    'Rendered HTML should include completed status class for completed step'
  );

  assert(
    html.includes('class="tree-icon in-progress"'),
    'Rendered HTML should include in-progress status class for in-progress step'
  );

  assert(
    html.includes('class="tree-icon pending"'),
    'Rendered HTML should include pending status class for pending step'
  );
});
