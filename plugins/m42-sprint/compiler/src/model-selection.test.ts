...
import type { CompiledProgress, CompiledTopPhase, CompiledPhase, CompiledStep } from './types.js';
...
/**
 * Create a temporary test environment with workflow and sprint files
 */
interface TestEnv {
  tempDir: string;
  workflowsDir: string;
...
function writeWorkflow(workflowsDir: string, name: string, content: object): void {
  const workflowPath = path.join(workflowsDir, `${name}.yaml`);
  fs.writeFileSync(workflowPath, yaml.dump(content));
}

function writeSprint(sprintDir: string, content: object): void {
...
test('compile: workflow-level model is applied to all phases', async () => {
  ...
  try {
    ...
    const progress = result.progress as CompiledProgress;
    ...
    for (const phase of progress.phases ?? []) {
      ...
      const phaseWithModel = phase as CompiledTopPhase & { model?: string };
...
test('compile: phase-level model overrides sprint-level model', async () => {
  ...
  try {
    ...
    const standardPhase = progress.phases?.find(p => p.id === 'standard-phase') as CompiledTopPhase & { model?: string };
    const explicitPhase = progress.phases?.find(p => p.id === 'explicit-phase') as CompiledTopPhase & { model?: string };
...
test('compile: step-level model has highest priority', async () => {
  ...
  try {
    ...
    const step1 = developPhase?.steps?.[0] as CompiledStep & { phases: Array<CompiledPhase & { model?: string }> };
    const step2 = developPhase?.steps?.[1] as CompiledStep & { phases: Array<CompiledPhase & { model?: string }> };
...
test('compile: resolves model with correct priority order', async () => {
  ...
  try {
    ...
    const step = developPhase?.steps?.[0] as CompiledStep & { phases: Array<CompiledPhase & { model?: string }> };

    // sub-phase-1 has explicit phase model: opus
    const subPhase1 = step.phases.find(p => p.id.includes('sub-phase-1'));
    assertEqual((subPhase1 as CompiledPhase & { model?: string }).model, 'opus', 'Sub-phase with explicit model should use opus');
    ...
    assertEqual((subPhase2 as CompiledPhase & { model?: string }).model, 'sonnet', 'Sub-phase without model should inherit sonnet');
  } finally {
    env.cleanup();
  }
});
