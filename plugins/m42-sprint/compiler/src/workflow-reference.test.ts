...
import { clearWorkflowCache } from './resolve-workflows.js';
import { validateWorkflowDefinition } from './validate.js';
import type { WorkflowDefinition, CompilerError } from './types.js';
...
function assertHasError(errors: CompilerError[], code: string, message?: string): void {
  const found = errors.find((e) => e.code === code);
  if (!found) {
    throw new Error(
      message ?? `Expected error with code '${code}', but found: ${errors.map((e) => e.code).join(', ') || 'none'}`
    );
  }
}
...
function writeWorkflow(ctx: TestContext, name: string, workflow: WorkflowDefinition): void {
...
test('Scenario 1: Workflow reference expands inline phases', async () => {
  ...
  try {
    ...
    assert(
      docsAnalyze!.prompt!.includes('Analyze'),
      `docs-analyze prompt should contain 'Analyze', got: ${docsAnalyze!.prompt}`
    );
  } finally {
...
test('Scenario 2: Phase IDs are prefixed with parent phase ID', async () => {
  ...
  try {
    ...
    assertArrayEqual(
      phaseIds,
      ['setup-init', 'setup-validate', 'setup-ready'],
      `Expected prefixed phase IDs`
    );
  } finally {
    cleanupTestContext(ctx);
  }
...
test('Scenario 3: Error when both prompt and workflow specified', () => {
  ...
  assertHasError(
    errors,
    'PROMPT_WORKFLOW_MUTUAL_EXCLUSIVE',
    'Should have PROMPT_WORKFLOW_MUTUAL_EXCLUSIVE error'
  );
  ...
  assert(
    ...
    `Error message should indicate mutual exclusivity: ${error!.message}`
  );
});
...
test('Scenario 4: Detect direct workflow self-reference', async () => {
  ...
  try {
    ...
    assert(
      cycleError!.message.includes('workflow-a'),
      `Cycle error should mention workflow-a: ${cycleError!.message}`
    );
  } finally {
    cleanupTestContext(ctx);
  }
...
test('Scenario 5: Detect indirect workflow cycle (A → B → A)', async () => {
  ...
  try {
    ...
    const mentionsBoth =
      cycleError!.message.includes('workflow-a') && cycleError!.message.includes('workflow-b');
...
test('Scenario 6: Enforce maximum workflow nesting depth', async () => {
  ...
  try {
    ...
    assert(
      depthError!.message.includes('5') || depthError!.message.toLowerCase().includes('depth'),
      `Error should mention depth limit: ${depthError!.message}`
    );
  } finally {
    cleanupTestContext(ctx);
  }
...
test('Scenario 7: Mixed inline and workflow-reference phases', async () => {
  ...
  try {
    ...
    assertArrayEqual(
      phaseIds,
      ['setup', 'docs-analyze', 'docs-reference', 'cleanup'],
...
test('Scenario 8: Nested workflow references within depth limit', async () => {
  ...
  try {
    ...
    assertArrayEqual(
      phaseIds,
      ['parent-prep', 'parent-child-ref-step1', 'parent-child-ref-step2'],
      'Nested phases should have cumulative prefixes'
    );
  } finally {
    cleanupTestContext(ctx);
  }
...
test('Edge case: Empty workflow reference should error', async () => {
  ...
  try {
    ...
    assert(
      !result.success || (result.progress?.phases?.length ?? 0) === 0,
      'Should handle empty workflow reference'
    );
  } finally {
    cleanupTestContext(ctx);
  }
