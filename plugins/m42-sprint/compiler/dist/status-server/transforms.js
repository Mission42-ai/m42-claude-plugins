 ...
 */
function computeElapsedIfNeeded(existingElapsed, startedAt, status) {
 ...
 */
function inferStepStatus(step, stepIndex, currentStepIndex, phaseStatus) {
 ...
 */
function buildStepNode(step, depth, stepIndex, currentStepIndex, phaseStatus) {
    // Infer status if current pointer info is provided
    const status = stepIndex !== undefined && currentStepIndex !== undefined && phaseStatus !== undefined
 ...
 */
function buildTopPhaseNode(topPhase, depth, phaseIndex, currentPhaseIndex, currentStepIndex) {
    const node = {
        id: topPhase.id,
    ...
    if (topPhase.steps) {
        ...
        node.children = topPhase.steps.map((s, idx) => buildStepNode(s, depth + 1, idx, stepCurrentIndex, topPhase.status));
    }
    return node;
}
/**
...
function buildPhaseTree(progress) {
    ...
    return (progress.phases ?? []).map((p, idx) => buildTopPhaseNode(p, 0, idx, currentPhaseIndex, currentStepIndex));
}
// ============================================================================
