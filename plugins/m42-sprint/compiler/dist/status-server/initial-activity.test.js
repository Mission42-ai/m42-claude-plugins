 ...
 */
function createActivityEvents(count) {
    const events = [];
 ...
 */
async function createTestSprintDir(options) {
...
function collectSSEEvents(port, timeout = 3000) {
...
test('BUG-003: Historical activity should be sent to newly connected SSE clients', async () => {
    ...
    try {
        ...
        assert(collected.statusUpdates.length > 0, 'Should receive at least 1 status-update on connect');
        // Verify we received log entry
        assert(collected.logEntries.length > 0, 'Should receive at least 1 log-entry on connect');
        // BUG-003: This assertion FAILS because historical activity is NOT sent
        // to newly connected SSE clients
        assert(collected.activityEvents.length > 0, `BUG-003: Historical activity should be sent to newly connected clients. ` +
        ...
        assert(collected.activityEvents.length === activityCount, `Should receive all ${activityCount} historical activity events. ` +
...
test('BUG-003: Activity events should include correct data structure', async () => {
    ...
    try {
        ...
        if (collected.activityEvents.length > 0) {
            const event = collected.activityEvents[0];
            ...
            assert(false, 'BUG-003: No activity events received. ' +
...
test('Empty activity file should not break initial connection', async () => {
    ...
    try {
        ...
        assert(collected.statusUpdates.length > 0, 'Should receive status-update even with empty activity file');
        // Activity events being empty is OK here
        assert(collected.activityEvents.length === 0, 'Should receive 0 activity events for empty activity file');
    }
    finally {
        cleanupTestDir(tmpDir);
    }
...
test('No activity file should not break initial connection', async () => {
    ...
    try {
        ...
        assert(collected.statusUpdates.length > 0, 'Should receive status-update even without activity file');
    }
    finally {
        cleanupTestDir(tmpDir);
    }
...
//# sourceMappingURL=initial-activity.test.js.map