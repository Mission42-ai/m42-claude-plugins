...
test('metrics section should display status breakdown (completed/failed/in-progress)', () => {
    ...
    const html = (0, dashboard_page_js_1.generateDashboardPage)(sprints, metrics, null);
    ...
    const hasCompletedMetric = metricsSection.includes('42 completed') ||
    ...
    const hasFailedMetric = metricsSection.includes('5 failed') ||
    ...
    const hasInProgressMetric = metricsSection.includes('3 in-progress') ||
    ...
    assert(hasCompletedMetric && hasFailedMetric && hasInProgressMetric, 'Metrics section should display status breakdown with labeled counts. ' +
...
test('metrics should be organized into categories', () => {
    ...
    const hasCategoryHeaders = metricsSection.includes('metrics-category') ||
    ...
    const hasStatusGroup = (metricsSection.includes('Status') && metricsSection.includes('Completed') && metricsSection.includes('Failed'));
    const hasPerformanceGroup = (metricsSection.includes('Performance') || metricsSection.includes('Timing'));
    const hasTrendGroup = metricsSection.includes('Trend') ||
    ...
    assert(hasMultipleGroups || (hasStatusGroup && hasPerformanceGroup), 'Metrics should be organized into categories with clear headers/groups. ' +
...
test('metrics should display workflow statistics', () => {
    ...
    const hasWorkflowInfo = html.includes('workflow') ||
        html.includes('Workflow') ||
        html.includes('60%'); // percentage of most common
    assert(hasMostCommonWorkflow && hasWorkflowInfo, 'Metrics should display workflow statistics. ' +
...
test('metrics should display average steps per sprint', () => {
    ...
    const hasStepMetricLabel = metricsSection.includes('Avg Steps') ||
        metricsSection.includes('Average Steps') ||
        metricsSection.includes('Steps/Sprint') ||
        metricsSection.includes('Steps Per Sprint');
    assert(hasAvgStepsValue && hasStepMetricLabel, 'Metrics section should display average steps per sprint (12.5) as a labeled metric. ' +
...
test('metrics should display trend information', () => {
    ...
    const hasTrendVisualization = html.includes('trend') ||
        html.includes('Trend') ||
        html.includes('chart') ||
        html.includes('Chart') ||
        html.includes('graph') ||
    ...
    const hasActivitySummary = html.includes('today') ||
        html.includes('this week') ||
        html.includes('recent') ||
        html.includes('W03'); // weekly trend key
    assert(hasTrendVisualization || hasActivitySummary, 'Metrics should display trend information or activity summary. ' +
...
test('success rate should include context', () => {
    ...
    const hasContext = html.includes('42/47') ||
        html.includes('42 of 47') ||
        html.includes('finished') ||
        html.includes('47 sprints') ||
        html.includes('title="'); // tooltip
    assert(hasSuccessRate && hasContext, 'Success rate (89%) should include context showing what it represents. ' +
...
test('metrics section should not include navigation elements', () => {
    const metrics = createTestMetrics();
    const sprints = createTestSprints();
    const html = (0, dashboard_page_js_1.generateDashboardPage)(sprints, metrics, 'active-sprint-123');
    ...
    const hasActiveSprintInMetrics = metricsSection.includes('Active Sprint') ||
        metricsSection.includes('active-sprint-123');
    assert(!hasActiveSprintInMetrics, '"Active Sprint" is navigation, not a metric. ' +
...
test('metrics should have visual hierarchy for importance', () => {
    ...
    assert(hasHierarchy, 'Metrics should have visual hierarchy to guide attention. ' +
...
test('metrics section should utilize at least 75% of calculated metrics', () => {
    ...
    const total = 12;
    // Check each metric
    if (html.includes(String(metrics.totalSprints)))
        utilized++; // totalSprints
    if (html.includes(String(metrics.completedSprints)))
        utilized++; // completedSprints
    if (html.includes(String(metrics.failedSprints)))
        utilized++; // failedSprints
    if (html.includes(String(metrics.inProgressSprints)))
        utilized++; // inProgressSprints
    if (html.includes(String(metrics.successRate)))
        utilized++; // successRate
    if (html.includes(metrics.averageDurationFormatted))
        utilized++; // averageDuration
    if (html.includes(String(metrics.averageStepsPerSprint)))
        utilized++; // averageStepsPerSprint
    if (metrics.mostCommonWorkflow && html.includes(metrics.mostCommonWorkflow))
        utilized++; // mostCommonWorkflow
    // Workflow stats (check if any workflow distribution is shown)
    if (metrics.workflowStats.some(w => html.includes(w.workflow) && html.includes(String(w.percentage))))
        utilized++;
    // Trend data (check if any trend info is shown)
    if (metrics.dailyTrend.some(d => html.includes(d.dateKey)))
        utilized++;
    if (metrics.weeklyTrend.some(w => html.includes(w.dateKey)))
        utilized++;
    ...
    const threshold = 75;
    assert(utilizationRate >= threshold, `Metrics utilization is ${Math.round(utilizationRate)}% (${utilized}/${total}). ` +
...
//# sourceMappingURL=dashboard-page.test.js.map