...
test('toStatusUpdate should include isStale flag', async () => {
    ...
    assert('isStale' in update.header, 'StatusUpdate header should have isStale field (RED: expected to fail)');
});
test('sprint with last-activity > 15 minutes should be marked stale', async () => {
    ...
    assertEqual(update.header.isStale, true, 'Sprint with last-activity > 15 min should be stale (RED: expected to fail)');
});
test('sprint with last-activity < 15 minutes should NOT be marked stale', async () => {
    ...
    assertEqual(update.header.isStale, false, 'Sprint with last-activity < 15 min should NOT be stale (RED: expected to fail)');
});
test('completed sprint should NOT be marked stale regardless of last-activity', async () => {
    ...
    assertEqual(update.header.isStale, false, 'Completed sprint should NOT be stale (RED: expected to fail)');
});
// ============================================================================
...
test('page.ts should have stale badge styling', async () => {
    ...
    const hasStaleBadgeCSS = html.includes('.stale') ||
        html.includes('status-badge.stale') ||
        html.includes('stale-badge');
    assert(hasStaleBadgeCSS, 'Page CSS should include stale badge styling (RED: expected to fail)');
});
test('page.ts should render Stale badge for stale sprints', async () => {
    ...
    const hasStaleRendering = html.includes('Stale') ||
        html.includes('stale') ||
        html.includes('isStale');
    assert(hasStaleRendering, 'Page JavaScript should render Stale badge (RED: expected to fail)');
});
test('page.ts should have Resume Sprint button', async () => {
    const { getPageHtml } = await import('./page.js');
    const html = getPageHtml();
    // Check for resume button
    const hasResumeButton = html.includes('Resume Sprint') ||
        html.includes('resume-sprint') ||
        html.includes('resumeSprint') ||
        html.includes('resume-btn');
    assert(hasResumeButton, 'Page should have Resume Sprint button (RED: expected to fail)');
});
test('Resume button should call resume API endpoint', async () => {
    const { getPageHtml } = await import('./page.js');
    const html = getPageHtml();
    // The resume button should call the /api/sprint/:id/resume endpoint
    const callsResumeApi = html.includes('/api/sprint') && html.includes('resume') ||
        html.includes("api/resume") ||
        html.includes("'/resume'");
    assert(callsResumeApi, 'Resume button should call resume API endpoint (RED: expected to fail)');
});
console.log('\nStale detection tests completed.\n');
//# sourceMappingURL=stale-detection.test.js.map