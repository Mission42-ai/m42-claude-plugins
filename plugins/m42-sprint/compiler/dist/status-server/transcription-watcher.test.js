 ...
 */
function createTextBlockStart(blockIndex = 0) {
 ...
 */
function createTextDelta(text, blockIndex = 0) {
    return JSON.stringify({
        type: 'stream_event',
        event: {
 ...
 */
function createTextBlockStop(blockIndex = 0) {
    return JSON.stringify({
        type: 'stream_event',
        event: {
...
test('Scenario 3: TranscriptionWatcher detects text content block start', async () => {
    ...
    try {
        const watcher = new transcription_watcher_js_1.TranscriptionWatcher(testDir, { debounceDelay: 50, maxEvents: 100 });
        ...
        const assistantEvents = receivedEvents.filter(e => e.type === 'assistant');
        assert(assistantEvents.length > 0, `TranscriptionWatcher should emit assistant event for text content. ` +
...
test('Scenario 3: Text block start is recognized distinct from tool_use', async () => {
    ...
    try {
        ...
        assert(assistantEvents.length > 0, `Should have assistant events, got ${assistantEvents.length}`);
        assert(toolEvents.length > 0, `Should have tool events (existing functionality), got ${toolEvents.length}`);
        watcher.close();
    }
    finally {
        cleanupTestDir(testDir);
...
test('Scenario 4: Text deltas are accumulated into complete message', async () => {
    ...
    try {
        ...
        assert(assistantEvents.length > 0, `Should have assistant events with accumulated text`);
        // Verify text is accumulated, not individual deltas
        const assistantEvent = assistantEvents[0];
        assert(assistantEvent.text === 'Hello world!', `Text should be accumulated. Expected 'Hello world!', got '${assistantEvent.text}'`);
        watcher.close();
    }
    finally {
        cleanupTestDir(testDir);
...
test('Scenario 4: Multiple separate text blocks produce separate events', async () => {
    ...
    try {
        ...
        assert(assistantEvents.length >= 2, `Should have 2 separate assistant events, got ${assistantEvents.length}`);
        watcher.close();
    }
    finally {
        cleanupTestDir(testDir);
...
test('Scenario 5: Rapid text deltas are debounced into single event', async () => {
    ...
    try {
        ...
        assert(assistantEvents.length === 1, `Rapid deltas should produce single debounced event. Got ${assistantEvents.length} events`);
        const event = assistantEvents[0];
        assert(event.text === 'One two three four five', `Debounced event should contain all accumulated text. Got '${event.text}'`);
        watcher.close();
    }
    finally {
        cleanupTestDir(testDir);
...
test('Scenario 5: Debounce delay is approximately 500ms for assistant text', async () => {
    ...
    try {
        ...
        assert(eventTimestamps.length > 0, `Should receive assistant event after debounce period`);
...
test('Edge case: Empty text deltas are handled gracefully', async () => {
    ...
    try {
        ...
        if (assistantEvents.length > 0) {
            const event = assistantEvents[0];
            assert(event.text === 'Hello', `Empty deltas should be handled, text should be 'Hello', got '${event.text}'`);
        }
        watcher.close();
    }
    finally {
...
test('Edge case: Text block without stop event still emits on debounce', async () => {
    ...
    try {
        ...
        assert(assistantEvents.length > 0, `Should emit assistant event after debounce even without content_block_stop`);
        watcher.close();
    }
    finally {
        cleanupTestDir(testDir);
...
test('Edge case: isThinking is true while accumulating, false when complete', async () => {
    ...
    try {
        ...
        if (assistantEvents.length > 0) {
            const event = assistantEvents[assistantEvents.length - 1];
            assert(event.isThinking === false, `Final assistant event should have isThinking=false after content_block_stop`);
        }
        watcher.close();
    }
    finally {
...
test('Backward compatibility: Tool events still emitted correctly', async () => {
    ...
    try {
        ...
        assert(toolEvents.length === 2, `Tool events should still work. Expected 2, got ${toolEvents.length}`);
        watcher.close();
    }
    finally {
        cleanupTestDir(testDir);
...
test('Backward compatibility: getRecentActivity includes assistant events', async () => {
    ...
    try {
        ...
        const assistantInRecent = recentActivity.filter(e => e.type === 'assistant');
        const toolInRecent = recentActivity.filter(e => e.type === 'tool');
        assert(assistantInRecent.length > 0, `getRecentActivity should include assistant events, got ${assistantInRecent.length}`);
        assert(toolInRecent.length > 0, `getRecentActivity should still include tool events, got ${toolInRecent.length}`);
        watcher.close();
    }
    finally {
        cleanupTestDir(testDir);
...
//# sourceMappingURL=transcription-watcher.test.js.map