 ...
 */
function getLogFilePath_CURRENT(sprintDir, phaseId) {
 ...
 */
function getLogFilePath_FIXED(sprintDir, phaseId) {
    const sanitized = phaseId.replace(/ > /g, '-').replace(/[^a-zA-Z0-9-_]/g, '_');
 ...
 */
function pathEscapesLogsDir(sprintDir, phaseId) {
...
test('sanitization prevents basic path traversal with ../', () => {
    ...
    assert(!result.includes('..'), `Path should not contain '..', got: ${result}`);
    assert(result.startsWith(path.join(sprintDir, 'logs')), `Path should start with logs dir, got: ${result}`);
});
...
test('BUG-006/012: should FAIL - no explicit path containment verification', () => {
    ...
    assert(hasExplicitPathCheck, 'getLogFilePath should include explicit path containment verification (defense-in-depth)');
});
// ============================================================================
...
test('FIXED implementation: throws on path traversal attempt', () => {
    ...
    assert(normalPath.includes('development-step-0-implement.log'), `Normal phase should work, got: ${normalPath}`);
});
test('FIXED implementation: rejects phaseId that could escape (if sanitization failed)', () => {
    ...
    function getLogFilePath_BROKEN_SANITIZATION(sprintDir, phaseId) {
    ...
    try {
        ...
        caughtError = true;
        assert(error instanceof Error && error.message === 'Invalid log path', `Expected 'Invalid log path' error, got: ${error}`);
    }
...
test('handles empty phaseId safely', () => {
    ...
    assert(result === path.join(sprintDir, 'logs', '.log'), `Empty phaseId should produce .log, got: ${result}`);
});
test('handles phaseId with only special characters', () => {
    const sprintDir = '/tmp/test-sprint';
    const result = getLogFilePath_CURRENT(sprintDir, '../../..');
    // All chars are replaced with underscores
    assert(result === path.join(sprintDir, 'logs', '________.log'), `Special-only phaseId should be all underscores, got: ${result}`);
});
test('handles normal phase ID format correctly', () => {
    const sprintDir = '/tmp/test-sprint';
    const result = getLogFilePath_CURRENT(sprintDir, 'development > step-0 > context');
    assert(result === path.join(sprintDir, 'logs', 'development-step-0-context.log'), `Normal phase ID should convert correctly, got: ${result}`);
});
 ...
 */
function createSignalFiles(sprintDir) {
 ...
 */
function getExistingSignalFiles(sprintDir) {
    const signals = ['.pause-requested', '.resume-requested', '.stop-requested', '.force-retry-requested'];
 ...
 */
function serverHasCleanupMethod() {
 ...
 */
function stopMethodCallsCleanup() {
    const distDir = __dirname;
    const srcDir = distDir.replace(/[/\\]dist[/\\]/, '/src/');
    const serverPath = path.join(srcDir, 'server.ts');
    if (fs.existsSync(serverPath)) {
...
test('BUG-010: StatusServer should have cleanupSignalFiles method', () => {
    ...
    assert(hasMethod, 'StatusServer should have a cleanupSignalFiles method that removes signal files');
});
// ============================================================================
...
test('BUG-010: stop() method should call cleanupSignalFiles', () => {
    ...
    assert(callsCleanup, 'stop() method should call cleanupSignalFiles to remove signal files');
});
// ============================================================================
...
test('BUG-010: signal files should be removed when server stops', async () => {
    ...
    try {
        ...
        assert(beforeStop.length === 4, `Expected 4 signal files before cleanup, got ${beforeStop.length}`);
        ...
        assert(afterStop.length === 0, `Expected 0 signal files after stop(), but found: ${afterStop.join(', ')}`);
    }
    finally {
...
test('BUG-010: leftover signal files should be cleaned on server start', async () => {
    ...
    try {
        ...
        assert(beforeStart.length === 4, `Expected 4 leftover signal files, got ${beforeStart.length}`);
        // Import and instantiate StatusServer
        // Use random high port to avoid conflicts
        const testPort = 30000 + Math.floor(Math.random() * 10000);
        const serverModule = await import('./server.js');
        ...
        assert(afterStart.length === 0, `Expected leftover signal files to be cleaned on start, but found: ${afterStart.join(', ')}`);
    }
    finally {
        // Cleanup test directory
        fs.rmSync(testDir, { recursive: true, force: true });
 ...
 */
function paginateSprints_CURRENT(allSprints, params) {
 ...
 */
function serverHasPaginationValidation() {
    const distDir = __dirname;
    const srcDir = distDir.replace(/[/\\]dist[/\\]/, '/src/');
    const serverPath = path.join(srcDir, 'server.ts');
    if (fs.existsSync(serverPath)) {
...
test('BUG-011: negative page should return error, not empty results with hasMore: true', () => {
    ...
    assert(result.page === -1, `Current code sets page to -1, got: ${result.page}`);
    // The validation should exist in the server
    const hasValidation = serverHasPaginationValidation();
    assert(hasValidation, 'Server should have pagination validation to reject negative page values');
});
// ============================================================================
...
test('BUG-014: page=0 should return error, not empty results', () => {
    ...
    assert(result.page === 0, `Current code sets page to 0, got: ${result.page}`);
    assert(result.sprints.length === 0, `Page 0 incorrectly returns empty results, got: ${result.sprints.length} items`);
    // The validation should exist in the server
    const hasValidation = serverHasPaginationValidation();
    assert(hasValidation, 'Server should have pagination validation to reject page=0');
});
// ============================================================================
...
test('BUG-015: non-numeric page should return error, not page: null/NaN', () => {
    ...
    // Verify buggy behavior
    assert(isNaN(result.page), `Current code sets page to NaN for non-numeric input, got: ${result.page}`);
    // The validation should exist in the server
    const hasValidation = serverHasPaginationValidation();
    assert(hasValidation, 'Server should have pagination validation to reject non-numeric page values');
});
// ============================================================================
...
test('BUG-016: negative limit should return error, not unexpected results', () => {
    ...
    // Verify buggy behavior
    assert(result.limit === -5, `Current code sets limit to -5, got: ${result.limit}`);
    // The validation should exist in the server
    const hasValidation = serverHasPaginationValidation();
    assert(hasValidation, 'Server should have pagination validation to reject negative limit values');
});
// ============================================================================
...
test('BUG-016: limit > 100 should be rejected or capped', () => {
    ...
    assert('error' in validation, `Limit > 100 should be rejected with error, got: ${JSON.stringify(validation)}`);
    // Check the server has this validation
    const hasValidation = serverHasPaginationValidation();
    assert(hasValidation, 'Server should have pagination validation to limit maximum page size');
});
// ============================================================================
...
`);
        ...
        try {
            ...
            const negativePageResponse = await fetch(`http://localhost:${testPort}/api/sprints?page=-1`);
            // Current bug: returns 200 with empty/wrong results
            // Fixed: should return 400 with error message
            assert(negativePageResponse.status === 400, `Negative page should return 400, got: ${negativePageResponse.status}`);
            // Test page=0 - should return 400
            const zeroPageResponse = await fetch(`http://localhost:${testPort}/api/sprints?page=0`);
            assert(zeroPageResponse.status === 400, `Page=0 should return 400, got: ${zeroPageResponse.status}`);
            // Test non-numeric page - should return 400
            const nonNumericPageResponse = await fetch(`http://localhost:${testPort}/api/sprints?page=abc`);
            assert(nonNumericPageResponse.status === 400, `Non-numeric page should return 400, got: ${nonNumericPageResponse.status}`);
            // Test negative limit - should return 400
            const negativeLimitResponse = await fetch(`http://localhost:${testPort}/api/sprints?limit=-5`);
            assert(negativeLimitResponse.status === 400, `Negative limit should return 400, got: ${negativeLimitResponse.status}`);
            // Test limit > 100 - should return 400
            const excessiveLimitResponse = await fetch(`http://localhost:${testPort}/api/sprints?limit=500`);
            assert(excessiveLimitResponse.status === 400, `Limit > 100 should return 400, got: ${excessiveLimitResponse.status}`);
        }
        finally {
            await server.stop();
        }
...
test('Valid pagination parameters should work correctly', () => {
    ...
    assert(page1.sprints.length === 2 && page1.sprints[0] === 'sprint-1', `Page 1 should return first 2 sprints, got: ${JSON.stringify(page1.sprints)}`);
    ...
    assert(page2.sprints.length === 2 && page2.sprints[0] === 'sprint-3', `Page 2 should return next 2 sprints, got: ${JSON.stringify(page2.sprints)}`);
    ...
    assert(page3.sprints.length === 1 && page3.sprints[0] === 'sprint-5', `Page 3 should return last sprint, got: ${JSON.stringify(page3.sprints)}`);
