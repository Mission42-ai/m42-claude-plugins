...
test('BUG-006: Page HTML should include a dedicated total duration element', async () => {
    ...
    assert(hasHeaderDuration, 'BUG-006: Page should have a prominent total duration display in the header area. ' +
...
test('BUG-006: Page CSS should style total duration prominently', async () => {
    ...
    const hasProminentDurationStyle = css.includes('.sprint-duration') ||
        css.includes('.total-duration') ||
        css.includes('.header-elapsed');
    // This should FAIL - no prominent duration styling exists
    assert(hasProminentDurationStyle, 'BUG-006: CSS should include prominent styling for total duration display. ' +
...
test('BUG-006: Page JavaScript should set elapsed for completed sprints without relying on timer', async () => {
    ...
    const setsElapsedForCompletedInHeader = js.includes("elements.elapsed.textContent = 'Total: ' + header.elapsed") ||
    ...
    assert(updateHeaderHandlesElapsed, 'BUG-006: updateHeader function should directly set elements.elapsed.textContent ' +
...
//# sourceMappingURL=total-duration.test.js.map