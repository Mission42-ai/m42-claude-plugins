 ...
 */
function simulateRenderLiveActivity(events) {
    if (events.length === 0) {
...
test('Scenario 6: Assistant messages render with chat bubble styling', () => {
    ...
    assert(html.includes('activity-assistant'), `Assistant messages should have 'activity-assistant' class. Got: ${html}`);
    assert(html.includes('activity-bubble'), `Assistant messages should have 'activity-bubble' class for chat styling`);
    assert(html.includes('Working on your request'), `Assistant message text should be displayed`);
});
test('Scenario 6: Assistant thinking indicator shows animation class', () => {
    ...
    assert(html.includes('activity-thinking'), `Thinking assistant messages should have 'activity-thinking' class for animation`);
});
test('Scenario 6: Full assistant message text is displayed (not truncated)', () => {
    ...
    const html = simulateRenderLiveActivity([assistantEvent]);
    assert(html.includes(longMessage), `Full assistant message should be displayed without truncation`);
});
// ============================================================================
...
test('Scenario 7: TodoWrite shows "Updated task list"', () => {
    ...
    assert(html.includes('Updated task list'), `TodoWrite should show 'Updated task list'. Got: ${html}`);
});
test('Scenario 7: Edit shows "Editing {filename}"', () => {
    ...
    assert(html.includes('Editing Button.tsx'), `Edit should show 'Editing {filename}'. Got: ${html}`);
});
test('Scenario 7: Read shows "Reading {filename}"', () => {
    ...
    assert(html.includes('Reading package.json'), `Read should show 'Reading {filename}'. Got: ${html}`);
});
test('Scenario 7: Write shows "Writing {filename}"', () => {
    ...
    assert(html.includes('Writing new-file.ts'), `Write should show 'Writing {filename}'. Got: ${html}`);
});
test('Scenario 7: Bash shows truncated command', () => {
    ...
    assert(html.includes('npm run build'), `Bash should show command. Got: ${html}`);
});
test('Scenario 7: Grep shows search pattern', () => {
    ...
    assert(html.includes('Searching: findUserById'), `Grep should show 'Searching: {pattern}'. Got: ${html}`);
});
// ============================================================================
...
test('Scenario 8: Tool entries have secondary styling class', () => {
    ...
    assert(html.includes('activity-tool'), `Tool entries should have 'activity-tool' class for secondary styling`);
});
test('Scenario 8: Tool entries are visually distinct from assistant messages', () => {
    ...
    assert(html.includes('activity-tool') && html.includes('activity-assistant'), `Tool and assistant entries should have distinct classes`);
    // Tool should use tool-desc, assistant should use bubble
    assert(html.includes('activity-tool-desc') && html.includes('activity-bubble'), `Tool should use tool-desc class, assistant should use bubble class`);
});
test('Scenario 8: Mixed tool and assistant events render in correct order', () => {
    ...
    assert(readingIdx < foundIdx && foundIdx < editingIdx, `Events should render in chronological order. ` +
...
//# sourceMappingURL=chat-activity.test.js.map