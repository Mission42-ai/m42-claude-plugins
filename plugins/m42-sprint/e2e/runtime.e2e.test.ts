...
import {
  ...
  createPauseFile,
  createSuccessMock,
  createFailOnIterationMock,
  createNeedsHumanMock,
...
// Type definitions for loop module
...
test('3.6: Phase failure should transition to blocked', async () => {
  ...
  try {
    ...
    if (result.finalState.status === 'blocked') {
      assert(
        result.finalState.error.includes('Simulated') || result.finalState.error.length > 0,
...
test('Runtime should handle empty phases array', async () => {
  ...
  try {
    ...
    const progress = readProgress(testDir);
    (progress as Record<string, unknown>).phases = [];
