...
// Import test helpers
import { test, assert, assertEqual, createTestDir, cleanupTestDir, copyFixture, compileSprint, readProgress, createSuccessMock, } from './test-helpers.js';
...
test('Activity types support assistant message type', async () => {
    ...
    const activityTypesPath = path.resolve(__dirname, '../../compiler/dist/status-server/activity-types.js');
    ...
    assert(typeof module.isActivityEvent === 'function', 'isActivityEvent should be exported');
    ...
    assert(module.isActivityEvent(assistantEvent), 'Should accept assistant event type');
...
test('calculateElapsed returns human-readable duration', async () => {
    const transformsPath = path.resolve(__dirname, '../../compiler/dist/status-server/transforms.js');
    ...
    assert(elapsed1.includes('1m') || elapsed1.includes('60s'), `Expected ~1m, got ${elapsed1}`);
    const elapsed2 = calculateElapsed(twoMinutesThirtySecondsAgo.toISOString());
    assert(elapsed2.includes('2m') && elapsed2.includes('30s'), `Expected ~2m 30s, got ${elapsed2}`);
...
test('Status update includes elapsed time for running sprint', async () => {
    ...
    assert(statusUpdate.header.elapsed !== undefined, 'Header should include elapsed time');
    assert(statusUpdate.header.elapsed.includes('h') ||
        statusUpdate.header.elapsed.includes('m'), `Elapsed should be human-readable, got: ${statusUpdate.header.elapsed}`);
});
// ============================================================================
...
test('Page generator includes sprint dropdown navigation', async () => {
    const pagePath = path.resolve(__dirname, '../../compiler/dist/status-server/page.js');
...
test('Runtime writes last-activity timestamp', async () => {
    ...
    try {
        ...
        assert(progress['last-activity'] !== undefined ||
            progress.stats?.['last-activity'] !== undefined, 'Should write last-activity timestamp');
    }
    finally {
...
`;
        fs.writeFileSync(path.join(testDir, '.claude', 'workflows', 'tdd-workflow.yaml'), workflowYaml, 'utf8');
        compileSprint(testDir);
        const progress = readProgress(testDir);
        ...
        assert(!!(progress.phases && progress.phases.length > 0), 'Should have phases');
        ...
        assert((phasesWithModel?.length ?? 0) > 0, 'Should have phases with model field');
        // Verify the model is 'opus' (inherited from sprint level)
        const phasesWithOpus = progress.phases?.filter((p) => p.model === 'opus');
        assert((phasesWithOpus?.length ?? 0) > 0, 'Should have phases with opus model inherited from sprint');
    }
    finally {
        cleanupTestDir(testDir);
    }
...
phases:
  ...
  - id: phase-b
    prompt: Do phase B
`;
        fs.writeFileSync(path.join(testDir, '.claude', 'workflows', 'simple-workflow.yaml'), workflowYaml, 'utf8');
        compileSprint(testDir);
        const progress = readProgress(testDir);
        ...
        const hasExpandedIds = phases.some((p) => p.id.includes('phase-a') || p.id.includes('phase-b'));
        assert(phases.length >= 2 || hasNestedPhases || hasExpandedIds, 'Workflow should be expanded into phases');
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('Operator processes pending requests', async () => {
    ...
    const criticalDecision = decisions.find((d) => d.requestId === 'req_001');
    assertEqual(criticalDecision?.decision, 'approve', 'Critical security should be approved');
    // Low priority should go to backlog
    const lowDecision = decisions.find((d) => d.requestId === 'req_002');
...
test('ProgressInjector injects steps at correct positions', async () => {
    ...
    try {
        copyFixture('minimal-sprint', testDir);
        compileSprint(testDir);
        const injectorPath = path.resolve(__dirname, '../../runtime/dist/progress-injector.js');
        ...
        const initialProgress = readProgress(testDir);
        ...
        // Verify injection
        const updatedProgress = readProgress(testDir);
        const updatedCount = updatedProgress.phases?.length ?? 0;
        assertEqual(updatedCount, initialCount + 1, 'Should have one more phase after injection');
        // Find injected step
        const injectedStep = updatedProgress.phases?.find((p) => p.id === 'injected-step');
        assert(injectedStep !== undefined, 'Should find injected step');
        assertEqual(injectedStep?.injected, true, 'Injected step should have injected: true marker');
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('Operator queue page renders all sections', async () => {
    const queuePagePath = path.resolve(__dirname, '../../compiler/dist/status-server/operator-queue-page.js');
    ...
    assert(html.includes('History') || html.includes('Decision'), 'Should have Decision History section');
    ...
    assert(html.includes('Critical security fix') || html.includes('reasoning'), 'Should show operator reasoning');
