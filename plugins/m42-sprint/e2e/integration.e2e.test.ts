...
test('4.2: Full bugfix workflow E2E - compile and run TDD cycle', async () => {
  ...
  try {
    ...
    const phases = progress.phases as Array<{
      id: string;
      steps?: Array<{ id: string; phases: unknown[] }>;
    }>;
    ...
    const options: LoopOptions = { maxIterations: 20, delay: 0, verbose: false };
    const result = await runLoop(testDir, options, mockDeps);
    ...
    assert(
      executedPrompts.some((p) => p.includes('failing tests')),
      'Should have red phase prompt'
    );
    assert(
      executedPrompts.some((p) => p.includes('Implement')),
      'Should have green phase prompt'
    );
    assert(
      executedPrompts.some((p) => p.includes('Refactor')),
      'Should have refactor phase prompt'
    );
  } finally {
    cleanupTestDir(testDir);
  }
...
test('Integration: Stats should be updated correctly', async () => {
  ...
  try {
    ...
    const finalProgress = readProgress(testDir) as {
      status: string;
      stats: {
    ...
    assert(
      finalProgress.stats['started-at'] !== null,
      'started-at should be set'
    );
