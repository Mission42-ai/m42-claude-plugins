...
### Added
- Worktree context warning injection into sprint prompts to ensure correct path resolution
- Worktree detection in CLI to prevent nested worktree creation
- `WorktreeContext` interface for worktree-aware prompt building
- `buildWorktreeContextWarning` function for worktree path rules injection
- Worktree context support in `buildPrompt`, `buildSimplePrompt`, `buildForEachPrompt`, and `buildParallelPrompt`

### Changed
- Claude runner now sets `PWD` and `GIT_WORK_TREE` environment variables for worktree execution
- Claude runner uses shell wrapper with explicit `cd` for reliable worktree cwd resolution
- CLI imports `getWorktreeInfo` for worktree state detection
