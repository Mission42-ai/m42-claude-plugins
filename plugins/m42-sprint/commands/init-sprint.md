...
1. **Sprint name argument provided**: The user must provide a sprint name as argument `$ARGUMENTS`
   - If empty, prompt: "Please provide a sprint name, e.g., `/init-sprint feature-auth` or `/init-sprint my-feature --ralph`"
...
Available workflows:
  ...
  - ralph (autonomous goal-driven - consider /init-sprint --ralph instead)
...
- `SPRINT.yaml` contains valid YAML appropriate for chosen mode:
  ...
  - Workflow mode: has `workflow:` and `collections:` keys
