...
import { ProgressInjector, } from './progress-injector.js';
// ============================================================================
// Test Utilities
// ============================================================================
...
async function assertThrows(fn, messageContains) {
    try {
        await fn();
...
function createMockProgressWithSteps() {
    return {
...
test('injectWorkflow: injects compiled workflow phases', async () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(workflowsDir, 'bugfix-workflow.yaml'), yaml.dump({
...
test('injected phase: preserves model if specified', async () => {
    ...
    try {
        ...
        const updated = readTestProgress(testDir);
        const injectedPhase = updated.phases.find(p => p.id === 'opus-step');
        assert(injectedPhase !== undefined, 'injected phase should exist');
...
//# sourceMappingURL=progress-injector.test.js.map