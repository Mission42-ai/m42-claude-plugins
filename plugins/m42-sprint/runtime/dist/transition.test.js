...
import { transition, advancePointer, calculateBackoff, getCurrentPhase, getCurrentStep, getCurrentSubPhase, } from './transition.js';
// ============================================================================
// Test Utilities
// ============================================================================
...
function assertContainsAction(actions, type) {
    const found = actions.some((a) => a.type === type);
    if (!found) {
        throw new Error(`Expected actions to contain ${type}, got: ${JSON.stringify(actions.map((a) => a.type))}`);
    }
}
function assertNoActions(result) {
    if (result.actions.length > 0) {
        throw new Error(`Expected no actions, got: ${JSON.stringify(result.actions.map((a) => a.type))}`);
    }
}
// ============================================================================
// Test Fixtures
...
function createContextWithSteps() {
    return {
        'sprint-id': 'test-sprint',
        status: 'in-progress',
...
function createContextWithRetry() {
...
function createContextWithOrchestration(autoApprove) {
    const ctx = createMinimalContext();
    ctx.orchestration = {
        enabled: true,
        autoApprove,
...
test('PHASE_COMPLETE: advances to next phase when more phases exist', () => {
    ...
    const event = { type: 'PHASE_COMPLETE', summary: 'Phase 0 done', phaseId: 'phase-0' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
    assertEqual(result.nextState.status, 'in-progress', 'Should remain in-progress');
    if (result.nextState.status === 'in-progress') {
...
test('PHASE_COMPLETE: transitions to completed when no more phases', () => {
    ...
    const event = { type: 'PHASE_COMPLETE', summary: 'All done', phaseId: 'phase-1' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
...
test('PHASE_COMPLETE: returns WRITE_PROGRESS action', () => {
    ...
    const event = { type: 'PHASE_COMPLETE', summary: 'Done', phaseId: 'phase-0' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
...
test('PHASE_COMPLETE: advances sub-phase when more sub-phases exist', () => {
    ...
    const event = { type: 'PHASE_COMPLETE', summary: 'Red done', phaseId: 'red' };
...
test('PHASE_COMPLETE: advances step when no more sub-phases', () => {
    ...
    const event = { type: 'PHASE_COMPLETE', summary: 'Refactor done', phaseId: 'refactor' };
    const context = createContextWithSteps();
    const result = transition(state, event, context);
    assertEqual(result.nextState.status, 'in-progress', 'Should remain in-progress');
    if (result.nextState.status === 'in-progress') {
...
test('PHASE_FAILED: retryable error schedules retry', () => {
    ...
    const event = { type: 'PHASE_FAILED', error: 'Network error', category: 'network', phaseId: 'phase-0' };
...
test('PHASE_FAILED: non-retryable error transitions to blocked', () => {
    ...
    const event = { type: 'PHASE_FAILED', error: 'Logic error', category: 'logic', phaseId: 'phase-0' };
    const context = createContextWithRetry();
    const result = transition(state, event, context);
...
test('PHASE_FAILED: blocked state includes error details', () => {
    ...
    const event = { type: 'PHASE_FAILED', error: 'Something broke', category: 'logic', phaseId: 'phase-0' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
...
test('PHASE_FAILED: blocked state returns WRITE_PROGRESS', () => {
    ...
    const event = { type: 'PHASE_FAILED', error: 'Error', category: 'validation', phaseId: 'phase-0' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
    assertContainsAction(result.actions, 'WRITE_PROGRESS');
});
...
test('PAUSE: transitions in-progress → paused', () => {
    ...
    const event = { type: 'PAUSE', reason: 'User requested' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
...
test('PAUSE: preserves current pointer in pausedAt', () => {
    ...
    const event = { type: 'PAUSE', reason: 'Break time' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
...
test('PAUSE: stores pause reason', () => {
    ...
    const event = { type: 'PAUSE', reason: 'Meeting time' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
    assert(result.nextState.status === 'paused', 'Must be paused');
    if (result.nextState.status === 'paused') {
...
test('PAUSE: returns WRITE_PROGRESS action', () => {
    ...
    const event = { type: 'PAUSE', reason: 'Test' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
    assertContainsAction(result.actions, 'WRITE_PROGRESS');
});
...
test('PROPOSE_STEPS: with autoApprove returns INSERT_STEP actions', () => {
    ...
    const result = transition(state, event, context);
    const insertActions = result.actions.filter((a) => a.type === 'INSERT_STEP');
...
test('HUMAN_NEEDED: transitions to needs-human', () => {
    ...
    const event = { type: 'HUMAN_NEEDED', reason: 'Decision required', details: 'Which approach?' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
...
test('HUMAN_NEEDED: stores reason and details', () => {
    ...
    const event = { type: 'HUMAN_NEEDED', reason: 'Clarification', details: 'Please advise on X' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
...
test('Invalid: PHASE_COMPLETE on not-started returns unchanged', () => {
    const state = { status: 'not-started' };
    const event = { type: 'PHASE_COMPLETE', summary: 'Done', phaseId: 'x' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
...
test('Invalid: PAUSE on completed returns unchanged', () => {
    ...
    const event = { type: 'PAUSE', reason: 'Try to pause' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
...
test('advancePointer: returns hasMore=false on last phase', () => {
    const current = { phase: 1, step: null, 'sub-phase': null };
...
test('advancePointer: advances sub-phase within step', () => {
    const current = { phase: 0, step: 0, 'sub-phase': 0 };
    const context = createContextWithSteps();
    const result = advancePointer(current, context);
...
test('advancePointer: advances step when sub-phases exhausted', () => {
    const current = { phase: 0, step: 0, 'sub-phase': 2 }; // Last sub-phase
    const context = createContextWithSteps();
    const result = advancePointer(current, context);
...
test('BREAKPOINT_REACHED: transitions in-progress → paused-at-breakpoint', () => {
    ...
    const event = { type: 'BREAKPOINT_REACHED', phaseId: 'review-checkpoint' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
...
test('BREAKPOINT_REACHED: preserves current pointer in pausedAt', () => {
    ...
    const event = { type: 'BREAKPOINT_REACHED', phaseId: 'phase-1' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
...
test('BREAKPOINT_REACHED: stores breakpoint phase ID', () => {
    ...
    const event = { type: 'BREAKPOINT_REACHED', phaseId: 'review-plans' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
    assert(result.nextState.status === 'paused-at-breakpoint', 'Must be paused-at-breakpoint');
    if (result.nextState.status === 'paused-at-breakpoint') {
...
test('BREAKPOINT_REACHED: returns WRITE_PROGRESS action', () => {
    ...
    const event = { type: 'BREAKPOINT_REACHED', phaseId: 'test' };
    const context = createMinimalContext();
    const result = transition(state, event, context);
    assertContainsAction(result.actions, 'WRITE_PROGRESS');
});
