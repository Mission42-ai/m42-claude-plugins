...
import { runLoop, recoverFromInterrupt, isTerminalState, } from './loop.js';
// ============================================================================
// Test Infrastructure
// ============================================================================
let testsPassed = 0;
...
test('runLoop should mark all phases as completed', async () => {
    ...
    try {
        ...
        const finalProgress = readProgressFile(testDir);
        const allCompleted = finalProgress.phases.every((p) => p.status === 'completed');
...
test('runLoop should stop after max iterations reached', async () => {
    ...
    try {
        ...
        assert(result.finalState.status === 'blocked' &&
            result.finalState.error.includes('iteration'), 'Error should mention iteration limit');
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('runLoop should include pause reason in state', async () => {
    ...
    try {
        ...
        const result = await runLoop(testDir, options, mockDeps);
        assert(result.finalState.status === 'paused' &&
            result.finalState.pauseReason.includes('PAUSE'), 'Pause reason should mention PAUSE file');
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('recoverFromInterrupt should restore from backup when checksum mismatch', async () => {
    ...
    try {
        ...
        const restoredProgress = yaml.load(restoredContent);
...
test('runLoop should populate human-needed reason', async () => {
    ...
    try {
        ...
        const result = await runLoop(testDir, options, mockDeps);
        assert(result.finalState.status === 'needs-human' &&
            result.finalState.reason === 'Missing API key', 'Reason should be populated from Claude result');
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('runLoop should respect delay option between iterations', async () => {
    ...
    try {
        ...
        writeProgress(testDir, progress);
        const delayMs = 100;
        const options = { maxIterations: 10, delay: delayMs, verbose: false };
        ...
        if (iterationTimes.length >= 2) {
            ...
            assert(elapsed >= delayMs * 0.9, // Allow 10% tolerance
...
test('runLoop should log iteration start and end when verbose', async () => {
    ...
    try {
        const progress = createTestProgress({ status: 'not-started' });
        writeProgress(testDir, progress);
        const options = { maxIterations: 10, delay: 0, verbose: true };
        const mockDeps = {
            runClaude: async () => ({
                success: true,
                output: '',
        ...
        const hasIterationLog = capturedLogs.some((log) => log.message.includes('iteration') || log.message.includes('Iteration'));
...
test('runLoop without deps should import runClaude internally (Bug 7 execution path)', async () => {
    ...
    try {
        ...
        if (result.finalState.status === 'blocked') {
            const blockedState = result.finalState;
            ...
            assert(blockedState.error !== 'Unknown error', `Bug 7: runLoop without deps returned "Unknown error" which means deps.runClaude was undefined. Error was: ${blockedState.error}`);
        }
...
test('BUG-018: runLoop should pass outputFile to runClaude for phase logs', async () => {
    ...
    try {
        ...
        assert(capturedOutputFile !== undefined, 'BUG-018: runClaude should receive outputFile parameter for phase logs');
        ...
        assert(capturedOutputFile.startsWith(transcriptionsDir), `BUG-018: outputFile should be in transcriptions directory. Got: ${capturedOutputFile}`);
        // Verify outputFile includes phase ID
        assert(capturedOutputFile.includes('phase-1') || capturedOutputFile.includes('phase_1'), `BUG-018: outputFile should include phase ID. Got: ${capturedOutputFile}`);
        // Verify outputFile ends with .log
        assert(capturedOutputFile.endsWith('.log'), `BUG-018: outputFile should end with .log. Got: ${capturedOutputFile}`);
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('BUG-018: runLoop should create transcriptions directory if it does not exist', async () => {
    ...
    try {
        ...
        assert(transcriptionsExistedDuringCall, 'BUG-018: transcriptions directory should be created before calling runClaude');
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('BUG-018: runLoop should generate unique log file per phase/step/sub-phase', async () => {
    ...
    try {
        ...
        assertEqual(capturedOutputFiles.length, 2, 'Should have called runClaude twice (once per phase)');
        // Both should have outputFile set
        assert(capturedOutputFiles[0] !== undefined && capturedOutputFiles[1] !== undefined, 'BUG-018: Both phases should have outputFile set');
        // Files should be different
        assert(capturedOutputFiles[0] !== capturedOutputFiles[1], `BUG-018: Each phase should have unique log file. Got same file for both: ${capturedOutputFiles[0]}`);
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('BUG-002: External writes during Claude execution are lost (race condition)', async () => {
    ...
    try {
        ...
        const mockDeps = {
            runClaude: async () => {
                ...
                if (claudeCallCount === 1) {
                    ...
                    const currentProgress = readProgressFile(testDir);
                    const phase2 = currentProgress.phases.find((p) => p.id === 'phase-2');
                    if (phase2) {
        ...
        const phase2Final = finalProgress.phases.find((p) => p.id === 'phase-2');
        assertEqual(phase2Final?.status, 'skipped', `BUG-002: External skip was lost! phase-2 status is "${phase2Final?.status}" but should be "skipped". ` +
    ...
    try {
        ...
        const mockDeps = {
            runClaude: async () => {
                ...
                throw new Error('BUG-002: Phase 2 was executed even though it was skipped! ' +
        ...
        const finalProgress = readProgressFile(testDir);
        const phase2Status = finalProgress.phases.find((p) => p.id === 'phase-2')?.status;
        assertEqual(phase2Status, 'skipped', `BUG-002: Phase 2 status should be 'skipped' but is '${phase2Status}'`);
        // Sprint should complete successfully
        assertEqual(result.finalState.status, 'completed', 'Sprint should complete after phase-1 since phase-2 was skipped');
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('BUG-002: Retry actions from status server are preserved', async () => {
    ...
    try {
        ...
        writeProgress(testDir, progress);
        const options = { maxIterations: 5, delay: 0, verbose: false };
        const mockDeps = {
            runClaude: async () => {
                ...
                if (!retryCountInjected) {
                    ...
                    const currentProgress = readProgressFile(testDir);
                    const phase1 = currentProgress.phases.find((p) => p.id === 'phase-1');
                    if (phase1) {
        ...
        const finalProgress = readProgressFile(testDir);
        const phase1 = finalProgress.phases.find((p) => p.id === 'phase-1');
        // The retry-count should be preserved (merged from external modification)
        assertEqual(phase1?.['retry-count'], 1, `BUG-002: retry-count was lost! Expected 1, got ${phase1?.['retry-count']}`);
...
test('BUG-001: Step status should be updated to in-progress when executing', async () => {
    ...
    try {
                    ...
                    steps: [
                        {
                            ...
                            status: 'pending',
                            phases: [
                                { id: 'context', status: 'pending', prompt: 'Gather context' },
                                { id: 'implement', status: 'pending', prompt: 'Implement feature' },
                            ],
                        },
                        {
                            id: 'step-1',
        ...
        const stepStatusesDuringExecution = [];
        const subPhaseStatusesDuringExecution = [];
        ...
        const mockDeps = {
            runClaude: async () => {
                ...
                const currentProgress = readProgressFile(testDir);
                const devPhase = currentProgress.phases?.[0];
                if (devPhase?.steps) {
        ...
        assert(hadInProgressStep, `BUG-001: Step status was NEVER set to 'in-progress' during execution!\n` +
        ...
        assert(hadInProgressSubPhase, `BUG-001: Sub-phase status was NEVER set to 'in-progress' during execution!\n` +
...
test('BUG-001: Completed step status should be preserved in PROGRESS.yaml', async () => {
    ...
    try {
        ...
        const finalProgress = readProgressFile(testDir);
        const devPhase = finalProgress.phases?.[0];
        // BUG-001: Verify step status was updated
        const step = devPhase?.steps?.[0];
        assert(step?.status === 'completed', `BUG-001: Step status should be 'completed' after execution!\n` +
        ...
        assert(subPhase?.status === 'completed', `BUG-001: Sub-phase status should be 'completed' after execution!\n` +
...
test('runLoop should parse operatorRequests from Claude JSON result', async () => {
    ...
    try {
        ...
        const finalProgress = readProgressFile(testDir);
        const operatorQueue = finalProgress['operator-queue'];
        // This test SHOULD FAIL until operator request system is implemented
        assert(operatorQueue !== undefined, 'STEP-5: operatorRequests should be queued in PROGRESS.yaml');
        assert(operatorQueue.length === 1, `STEP-5: Should have 1 request in queue, got ${operatorQueue?.length}`);
        assertEqual(operatorQueue[0].id, 'req_001', 'STEP-5: Request ID should be preserved');
        assertEqual(operatorQueue[0].status, 'pending', 'STEP-5: Request status should be "pending"');
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('runLoop should trigger operator for critical priority requests', async () => {
    ...
    try {
        ...
        assert(criticalRequest?.status !== 'pending', 'STEP-5: Critical request should not remain pending - operator should process immediately');
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('runLoop should add discovered-in and created-at to queued requests', async () => {
    ...
    try {
        ...
        assertEqual(request?.['discovered-in'], 'test-phase', 'STEP-5: discovered-in should be set to current phase ID');
        // Check created-at is a valid timestamp within our execution window
        assert(request?.['created-at'] !== undefined, 'STEP-5: created-at should be set');
        assert(request['created-at'] >= beforeTime && request['created-at'] <= afterTime, 'STEP-5: created-at should be within execution time window');
    }
    finally {
        cleanupTestDir(testDir);
    }
...
test('runLoop should handle empty operatorRequests array', async () => {
    ...
    try {
        ...
        const operatorQueue = finalProgress['operator-queue'];
        assert(operatorQueue === undefined || operatorQueue.length === 0, 'STEP-5: Empty operatorRequests should not create queue entries');
    }
    finally {
        cleanupTestDir(testDir);
    }
    ...
    try {
        ...
        const progress = {
            ...
            phases: [{
                    ...
                    status: 'pending',
                    steps: [
                        { id: 'step-a', status: 'pending', prompt: 'Step A', phases: [] },
                        { id: 'step-b', status: 'pending', prompt: 'Step B', phases: [] },
                    ],
                }],
            stats: {
                'started-at': null,
...
test('runLoop should fall back to sequential when parallel disabled', async () => {
    ...
    try {
        ...
        progress['parallel-execution'] = { enabled: false };
        writeProgress(testDir, progress);
    ...
    try {
        ...
        const progress = {
            ...
            phases: [{
                    ...
                    steps: [
                        { id: 'step-a', status: 'pending', prompt: 'Step A', phases: [] },
                        { id: 'step-b', status: 'pending', prompt: 'Step B', phases: [], 'depends-on': ['step-a'] },
                    ],
                }],
            stats: {
                'started-at': null,
            ...
            'parallel-execution': {
                enabled: true,
                maxConcurrency: 2,
                onDependencyFailure: 'fail-phase',
            },
            'dependency-graph': [{
                    'phase-id': 'dev',
                    nodes: [
