...
test('sprint-loop.sh should not exist', () => {
    assert(!fileExists('scripts/sprint-loop.sh'), 'scripts/sprint-loop.sh still exists - should be deleted');
});
test('build-sprint-prompt.sh should not exist', () => {
    assert(!fileExists('scripts/build-sprint-prompt.sh'), 'scripts/build-sprint-prompt.sh still exists - should be deleted');
});
test('build-parallel-prompt.sh should not exist', () => {
    assert(!fileExists('scripts/build-parallel-prompt.sh'), 'scripts/build-parallel-prompt.sh still exists - should be deleted');
});
test('preflight-check.sh should not exist', () => {
    assert(!fileExists('scripts/preflight-check.sh'), 'scripts/preflight-check.sh still exists - should be deleted');
});
// ============================================================================
...
test('test-sprint-features.sh should exist', () => {
    assert(fileExists('scripts/test-sprint-features.sh'), 'scripts/test-sprint-features.sh should be preserved');
});
test('test-skip-spawned.sh should exist', () => {
    assert(fileExists('scripts/test-skip-spawned.sh'), 'scripts/test-skip-spawned.sh should be preserved');
});
test('test-skip-parallel-task-id.sh should exist', () => {
    assert(fileExists('scripts/test-skip-parallel-task-id.sh'), 'scripts/test-skip-parallel-task-id.sh should be preserved');
});
test('test-normal-subphase.sh should exist', () => {
    assert(fileExists('scripts/test-normal-subphase.sh'), 'scripts/test-normal-subphase.sh should be preserved');
});
// ============================================================================
...
test('run-sprint.md should not reference sprint-loop.sh', () => {
    assert(!fileContains('commands/run-sprint.md', /sprint-loop\.sh/), 'commands/run-sprint.md still references sprint-loop.sh');
});
test('run-sprint.md should not reference build-sprint-prompt', () => {
    assert(!fileContains('commands/run-sprint.md', /build-sprint-prompt/), 'commands/run-sprint.md still references build-sprint-prompt');
});
test('pause-sprint.md should not reference sprint-loop.sh', () => {
    assert(!fileContains('commands/pause-sprint.md', /sprint-loop\.sh/), 'commands/pause-sprint.md still references sprint-loop.sh');
});
test('resume-sprint.md should not reference sprint-loop.sh', () => {
    assert(!fileContains('commands/resume-sprint.md', /sprint-loop\.sh/), 'commands/resume-sprint.md still references sprint-loop.sh');
});
test('stop-sprint.md should not reference sprint-loop.sh', () => {
    assert(!fileContains('commands/stop-sprint.md', /sprint-loop\.sh/), 'commands/stop-sprint.md still references sprint-loop.sh');
});
// ============================================================================
...
test('run-sprint.md should reference TypeScript CLI', () => {
    ...
    assert(hasNodeRuntime || hasSprintRun, 'commands/run-sprint.md should reference TypeScript CLI (node runtime/dist/cli.js or "sprint run")');
});
// ============================================================================
...
test('README.md should not require yq installation', () => {
    assert(!fileContains('README.md', /brew install yq/), 'README.md still mentions "brew install yq"');
    assert(!fileContains('README.md', /snap install yq/), 'README.md still mentions "snap install yq"');
});
test('README.md should document Node.js requirement', () => {
    assert(fileContains('README.md', /Node\.js|node\.js|nodejs/i), 'README.md should mention Node.js requirement');
});
// ============================================================================
// Summary
// ============================================================================
