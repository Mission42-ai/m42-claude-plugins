...
import { runClaude, extractJson, categorizeError, buildArgs, SPRINT_RESULT_SCHEMA, } from './claude-runner.js';
// ============================================================================
// Test Utilities
// ============================================================================
...
function assertArrayContainsSequence(arr, ...sequence) {
...
\`\`\``;
    const result = extractJson(output);
    assertEqual(result.data.nested.value, 42);
});
...
\`\`\``;
    const result = extractJson(output);
    assertEqual(result.multiline, true);
});
...
\`\`\``;
    const result = extractJson(output);
    assertEqual(result.realJson, true);
});
// ============================================================================
...
test('SPRINT_RESULT_SCHEMA: has correct structure for sprint results', () => {
    ...
    const required = SPRINT_RESULT_SCHEMA.required;
    assertArrayContains(required, 'status');
    assertArrayContains(required, 'summary');
    // Verify properties exist
    const properties = SPRINT_RESULT_SCHEMA.properties;
...
test('SPRINT_RESULT_SCHEMA: humanNeeded has nested structure', () => {
    const properties = SPRINT_RESULT_SCHEMA.properties;
    const humanNeeded = properties.humanNeeded;
...
test('ErrorCategory: all categories are valid', () => {
    const categories = ['network', 'rate-limit', 'timeout', 'validation', 'logic'];
...
//# sourceMappingURL=claude-runner.test.js.map