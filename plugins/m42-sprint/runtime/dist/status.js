 ...
 */
export function listAllWorktrees(cwd) {
 ...
 */
export function findSprintsInWorktree(worktreePath) {
 ...
 */
export function readSprintProgress(sprintDir) {
    const progressFile = path.join(sprintDir, 'PROGRESS.yaml');
    try {
        if (!fs.existsSync(progressFile)) {
 ...
 */
export function discoverSprints(cwd, options = {}) {
 ...
 */
export function formatSprintStatus(sprint, useColor = true) {
    const lines = [];
    const indent = '  ';
 ...
 */
export function formatCrossWorktreeStatus(result, useColor = true) {
    const lines = [];
    // Header
    ...
    const sprintsByWorktree = new Map();
    ...
    // Summary
    const activeCount = result.sprints.filter(s => s.progress?.status === 'in-progress').length;
 ...
 */
export function formatCurrentWorktreeStatus(result, useColor = true) {
    const lines = [];
