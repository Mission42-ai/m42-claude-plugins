...
import { getWorktreeInfo, getRepoRoot, checkBranchExists, } from './worktree.js';
 ...
 */
export function getLockDir(cwd) {
 ...
 */
export function ensureLockDir(cwd) {
 ...
 */
export function generateLockFileName(operation, worktreeId, suffix) {
 ...
 */
export function parseLockFile(lockPath) {
    try {
 ...
 */
export function createLockFile(lockDir, lockInfo) {
 ...
 */
export function releaseLock(lockPath, worktreeId) {
    const existing = parseLockFile(lockPath);
    if (!existing) {
 ...
 */
export function acquireLock(cwd, operation, options = {}) {
 ...
 */
export function isLockStale(lockInfo, maxAgeMs = DEFAULT_STALE_LOCK_AGE_MS) {
 ...
 */
export function cleanupStaleLocks(cwd, maxAgeMs = DEFAULT_STALE_LOCK_AGE_MS) {
    const lockDir = getLockDir(cwd);
 ...
 */
export function listActiveLocks(cwd) {
    const lockDir = getLockDir(cwd);
    if (!lockDir || !fs.existsSync(lockDir)) {
        return [];
    }
 ...
 */
export function checkBranchConflict(options) {
    ...
    try {
        ...
        for (const sprint of sprintsResult.sprints) {
            ...
            if (sprint.worktree.branch === branchName) {
                const isActive = sprint.progress?.status === 'in-progress' ||
 ...
 */
export function checkSprintConflicts(cwd, branchName, sprintId) {
    ...
    const conflictingLock = activeLocks.find((lock) => lock.branch === branchName &&
 ...
 */
export function suggestAlternativeBranch(baseBranch, cwd) {
    const info = getWorktreeInfo(cwd);
 ...
 */
export function formatConflictSuggestions(sprintId, worktreePath) {
    return `Suggestions:
 ...
 */
export function formatFullConflictMessage(params) {
