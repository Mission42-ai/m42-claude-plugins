...
function readProgressFile(sprintDir: string): CompiledProgress & { 'last-activity'?: string; 'interrupted-at'?: string } {
  const progressPath = path.join(sprintDir, 'PROGRESS.yaml');
  const content = fs.readFileSync(progressPath, 'utf8');
  return yaml.load(content) as CompiledProgress & { 'last-activity'?: string; 'interrupted-at'?: string };
}
...
test('runLoop should write last-activity timestamp each iteration', async () => {
  ...
  try {
    ...
    assert(
      'last-activity' in finalProgress,
      'PROGRESS.yaml should contain last-activity field (RED: expected to fail)'
    );

    // Verify it's a valid ISO timestamp
    const lastActivity = finalProgress['last-activity'];
    assert(
      lastActivity !== undefined && !isNaN(Date.parse(lastActivity)),
...
test('last-activity should be updated on each iteration', async () => {
  ...
  try {
    ...
    assert(
      timestamps.length >= 2,
      `Expected at least 2 timestamps, got ${timestamps.length} (RED: expected to fail)`
    );
    ...
    if (timestamps.length >= 2) {
      ...
      assert(
        t2 >= t1,
        'Second last-activity timestamp should be >= first'
      );
    }
  } finally {
    cleanupTestDir(testDir);
...
test('SIGTERM handler should mark sprint as interrupted', async () => {
  ...
  assert(
    hasSigtermHandler,
    "loop.ts should handle SIGTERM signal (RED: expected to fail)"
  );

  // Check that it sets status to 'interrupted'
  const hasInterruptedStatus =
  ...
  assert(
    ...
    "SIGTERM handler should set status to 'interrupted' (RED: expected to fail)"
  );
});
...
test('SIGTERM handler should write interrupted-at timestamp', async () => {
  ...
  assert(
    hasInterruptedAt,
    "SIGTERM handler should write interrupted-at timestamp (RED: expected to fail)"
  );
});
...
test('SIGINT handler should mark sprint as interrupted', async () => {
  ...
  assert(
    hasSigintHandler,
    "loop.ts should handle SIGINT signal (RED: expected to fail)"
  );
});
...
test('signal handlers should clean up before exit', async () => {
  ...
  const hasCleanupAndExit =
    (sourceCode.includes('process.exit') || sourceCode.includes('graceful')) &&
    (sourceCode.includes('writeProgress') || sourceCode.includes('writeProgressAtomic'));

  assert(
    hasCleanupAndExit,
    "Signal handlers should write progress and exit gracefully (RED: expected to fail)"
  );
});
