...
function createMockRequest(overrides: Partial<OperatorRequest> = {}): OperatorRequest {
  return {
    id: 'req_test123',
    title: 'Test Request',
...
function createMockQueuedRequest(overrides: Partial<QueuedRequest> = {}): QueuedRequest {
  return {
    ...createMockRequest(),
    status: 'pending',
...
function createMockDecision(overrides: Partial<OperatorDecision> = {}): OperatorDecision {
  return {
...
test('end-of-phase position resolves correctly', () => {
  const injection = {
    position: { type: 'end-of-phase' as const },
    prompt: 'Cleanup task',
    idPrefix: 'cleanup',
  };
