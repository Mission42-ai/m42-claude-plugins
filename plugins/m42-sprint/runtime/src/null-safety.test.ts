...
function assertNotThrows(fn: () => void, message?: string): void {
  try {
    fn();
  } catch (error) {
 ...
 */
function createContextWithEmptyPhases(): CompiledProgress {
  return {
 ...
 */
function createContextWithIndexOutOfBounds(): CompiledProgress {
  return {
    'sprint-id': 'test-sprint',
    status: 'in-progress',
 ...
 */
function createContextWithStepWithoutSubPhases(): CompiledProgress {
  return {
    'sprint-id': 'test-sprint',
    status: 'in-progress',
 ...
 */
function createContextWithStepUndefinedPhases(): CompiledProgress {
  return {
    'sprint-id': 'test-sprint',
    status: 'in-progress',
    current: { phase: 0, step: null, 'sub-phase': null },
    ...
    phases: [
      {
        ...
        steps: [
          ...
          } as CompiledStep, // Type assertion to allow missing phases
        ],
      },
    ],
  };
 ...
 */
function createContextAdvancingToStepsNoSubPhases(): CompiledProgress {
  return {
    'sprint-id': 'test-sprint',
    status: 'in-progress',
...
test('BUG-004: advancePointer handles out-of-bounds phase index without throwing', () => {
  const current: CurrentPointer = { phase: 99, step: null, 'sub-phase': null };
...
test('BUG-008: advancePointer sets sub-phase=null when next step has empty phases array', () => {
  ...
  const current: CurrentPointer = { phase: 0, step: 0, 'sub-phase': 1 };
  ...
  assertEqual(
    result.nextPointer['sub-phase'],
    null,
    'BUG-008: sub-phase should be null when next step has no sub-phases (empty array)'
  );
});
...
test('BUG-008: createPointerForPhase sets sub-phase=null for phase with steps but no sub-phases', () => {
  ...
  assertEqual(
    result.nextPointer['sub-phase'],
    null,
    'BUG-008: sub-phase should be null when step has empty phases array'
  );
});
...
test('BUG-009: pointer for phase with steps but undefined phases in first step', () => {
  ...
  let result: { nextPointer: CurrentPointer; hasMore: boolean } | undefined;
  assertNotThrows(() => {
  ...
  if (result !== undefined) {
    ...
    assertEqual(
      result.nextPointer['sub-phase'],
      null,
      'BUG-009: sub-phase should be null when step.phases is undefined'
    );
  }
});
...
test('Edge case: advancing through multiple phases with varying structures', () => {
  ...
  let current: CurrentPointer = { phase: 0, step: null, 'sub-phase': null };
  ...
  assertEqual(
    result.nextPointer['sub-phase'],
    null,
    'First advance: sub-phase should be null for steps without sub-phases'
  );
});
