...
import {
  runLoop,
  executePerIterationHooks,
  replaceHookTemplateVars,
  LoopOptions,
  PerIterationHook,
  HookTask,
} from './loop.js';
...
type ProgressWithHooks = CompiledProgress & {
  'per-iteration-hooks'?: PerIterationHook[];
  'hook-tasks'?: HookTask[];
};

function createTestProgress(overrides: Partial<ProgressWithHooks> = {}): ProgressWithHooks {
  return {
    ...
    ...overrides,
  } as ProgressWithHooks;
}

function writeProgress(sprintDir: string, progress: ProgressWithHooks): void {
...
function readProgressFile(sprintDir: string): ProgressWithHooks {
  const progressPath = path.join(sprintDir, 'PROGRESS.yaml');
  const content = fs.readFileSync(progressPath, 'utf8');
  return yaml.load(content) as ProgressWithHooks;
}
...
test('replaceHookTemplateVars should replace multiple variables', () => {
  ...
  assertEqual(
    result,
    '/m42-signs:extract /sprints/test/transcriptions/phase-0.log --sprint test-sprint --iteration 5 --phase qa'
  );
});
...
test('runLoop should execute enabled hook after iteration', async () => {
  ...
  try {
    ...
    let callCount = 0;
    const mockDeps = {
      runClaude: async (opts: { prompt: string }) => {
        callCount++;
        if (callCount === 1) {
    ...
    assert(
      hookPrompt.includes('Extract learnings from'),
      `Hook prompt should contain expected text. Got: ${hookPrompt}`
    );
    // Verify template variable was replaced (should not contain $ITERATION_TRANSCRIPT)
    assert(
      !hookPrompt.includes('$ITERATION_TRANSCRIPT'),
      'Template variable $ITERATION_TRANSCRIPT should be replaced'
    );
  } finally {
...
test('runLoop should track hook task in progress', async () => {
  ...
  try {
    ...
    assert(
      finalProgress['hook-tasks'] !== undefined,
      'hook-tasks should be created in progress'
    );

    assert(
      finalProgress['hook-tasks']!.length > 0,
      'hook-tasks should have entries'
    );
...
test('runLoop should replace template variables in hook prompt', async () => {
  ...
  try {
    ...
    assert(
      !capturedPrompt.includes('$ITERATION_TRANSCRIPT'),
      '$ITERATION_TRANSCRIPT should be replaced'
    );
    assert(
      !capturedPrompt.includes('$SPRINT_ID'),
      '$SPRINT_ID should be replaced'
    );
    assert(
      !capturedPrompt.includes('$PHASE_ID'),
      '$PHASE_ID should be replaced'
    );

    // Verify values were substituted correctly
    assert(
      capturedPrompt.includes('my-test-sprint'),
      'Sprint ID should be substituted'
    );
    assert(
      capturedPrompt.includes('test-phase'),
      'Phase ID should be substituted'
    );
    assert(
      capturedPrompt.includes('.log'),
      'Transcript path should be substituted'
    );
  } finally {
    cleanupTestDir(testDir);
  }
...
test('runLoop should execute hook for each iteration', async () => {
  ...
  try {
    ...
    const finalProgress = readProgressFile(testDir);
    assertEqual(
      finalProgress['hook-tasks']?.length,
      2,
      'Hook tasks should be tracked for each iteration'
    );
  } finally {
    cleanupTestDir(testDir);
  }
...
test('runLoop should log hook execution when verbose', async () => {
  ...
  try {
    ...
    const options: LoopOptions = { maxIterations: 1, delay: 0, verbose: true };
    ...
    const hasHookLog = capturedLogs.some(
      (log) => log.message.includes('hook') || log.message.includes('Hook')
    );
