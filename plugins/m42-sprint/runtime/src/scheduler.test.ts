...
import {
  ...
  type CompiledDependencyGraph,
  type CompiledTopPhase,
  type CompiledStep,
} from './scheduler.js';
...
function assertArrayEqual<T>(actual: T[], expected: T[], message?: string): void {
  const msg = message || `Expected ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`;
  if (actual.length !== expected.length) throw new Error(msg);
  for (let i = 0; i < actual.length; i++) {
    if (actual[i] !== expected[i]) throw new Error(msg);
  }
}
...
function createStep(id: string, status: 'pending' | 'in-progress' | 'completed' | 'failed' | 'skipped' = 'pending', dependsOn?: string[]): CompiledStep {
  return {
    id,
    prompt: `Task for ${id}`,
    status,
...
function createPhase(id: string, steps: CompiledStep[]): CompiledTopPhase {
  return {
    id,
    status: 'pending',
    steps,
...
function createProgress(phases: CompiledTopPhase[], dependencyGraphs?: CompiledDependencyGraph[]): CompiledProgress {
  return {
...
test('injectStep with dependencies', () => {
  ...
  assert(
    scheduler.getNode('step-1')?.dependents.includes('step-new') ?? false,
    'step-1 should have step-new as dependent'
  );
});
