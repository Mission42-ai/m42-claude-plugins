...
import {
  substituteWorktreeVars,
  extractSprintName,
  extractDate,
  createWorktree,
  buildWorktreeProgressSection,
  formatWorktreeSuccessMessage,
...
const testVars: WorktreeVariables = {
...
test('Integration: two worktrees with same sprint name should have separate isolation metadata', () => {
  ...
  try {
    ...
    assert(meta1['worktree-id'] !== meta2['worktree-id'],
      'Same sprint name in different worktrees should have different worktree IDs');
    ...
    assert(meta1['worktree-path'] !== meta2['worktree-path'],
      'Worktree paths should be different');

    // Validate isolation for each sprint
    assertEqual(validateWorktreeIsolation(sprintDir1, worktreePath1), true,
      'Sprint 1 should be valid in worktree 1');
    assertEqual(validateWorktreeIsolation(sprintDir2, worktreePath2), true,
      'Sprint 2 should be valid in worktree 2');

    // Cross-worktree validation should fail
    assertEqual(validateWorktreeIsolation(sprintDir1, worktreePath2), false,
      'Sprint 1 should NOT be valid in worktree 2');
    assertEqual(validateWorktreeIsolation(sprintDir2, worktreePath1), false,
      'Sprint 2 should NOT be valid in worktree 1');
...
test('Integration: .claude/sprints/ is local to each worktree', () => {
  ...
  try {
    ...
    assert(fs.existsSync(path.join(mainSprintDir, 'main-marker.txt')),
      'Main sprint dir should have its marker');
    assert(!fs.existsSync(path.join(mainSprintDir, 'wt-marker.txt')),
      'Main sprint dir should NOT have worktree marker');

    assert(fs.existsSync(path.join(wtSprintDir, 'wt-marker.txt')),
      'Worktree sprint dir should have its marker');
    assert(!fs.existsSync(path.join(wtSprintDir, 'main-marker.txt')),
      'Worktree sprint dir should NOT have main marker');
    ...
    assert(normalizedMainPath !== normalizedWtPath,
      'Worktree paths should be different');
